from inverse_opt_stopping.q_networks import OfflineQNetwork, OfflineQNetwork_orig, gNetworkRNN
import torch
...
class IQ_Agent:
    def __init__(self, obs_dim=None, action_dim=2, divergence='kl_fix', approx_g=True, approx_dynamics=True, gamma=0.99, epsilon=0.1, q_lr=0.001,
        ...
        if approx_dynamics:
            ...
            if approx_g:
                g_net = gNetworkRNN(obs_dim=obs_dim, action_dim=action_dim, gamma=self.gamma,
                                    device=device) 
                ...
                if oversampling is not None:
                    raise ValueError('Cannot do oversampling when approximating g')
            else:
    ...
    def g_update(self, batch, epoch):
        obs = batch[0]
        y, y_next, y_prev, obs_hist = batch[-4:]
        obs_ext = torch.hstack([obs, y]).clone()
        # obs_ext_prev = torch.hstack([obs, y_prev]).clone()
        self.g_net.train()
        
        # g_phi = self.g_net(obs_ext_prev.to(self.device))
        # print(f'g_update obs_hist.shape: {obs_hist.shape}')
        g_phi = self.g_net(obs_hist.to(self.device))[:,-1,:].reshape(-1,1)
        g_iq = self.get_rewards(obs_ext,y_prev)[1]
        # print(f'g_phi.shape: {g_phi.shape}')
        # print(f'g_iq.shape: {g_iq.shape}')
        # print()
    ...
    def iq_update(self, batch, epoch, cs, update=False):
        ...
        if self.approx_dynamics:
            state_loss = self.env_loss(next_state_k.to(self.device), next_obs_k[:,:].to(self.device)).to(self.device)
            ...
            for p, path_id in enumerate(np.unique(test_memory['path_ids'].astype(int))):
                ...
                for time_id in sample['time_ids']:
                    ...
                    with torch.no_grad():
                        ...
                        base_state_hist = sample['state_mem'][sample['time_ids']<=time_id]
                        if self.approx_g:
                            # ext_state = torch.hstack([base_state.clone(), torch.from_numpy(np.array(y_hist[-1])).reshape(1,-1)])
                            irl_g = self.g_net(base_state_hist)
    ...
    def train(self, mem, batch_size=250, n_epoches=5000, verbose=1):
        ...
        for epoch in np.arange(n_epoches):
            ...
            train_ids = random_batch_split(np.arange(len(train_batch['time_ids'])), batch_size)
            mem_keys = list(memory.keys())
                ...
                if self.approx_g:
                    ...
                    for k in range(len(path_ids)):
                        ...
                        for time_id in time_ids_hist:
                            ...
                            # st_hist_all.append(st_hist)
                            # print(f"memory['next_state_mem'][new_mask_next].shape: {memory['next_state_mem'][new_mask_next].shape}")
                            st_next_hist = (torch.from_numpy(np.array(memory['next_state_mem'])[new_mask_next][-1,:])-self.mean)/self.std
                            # print(f'st_next_hist.shape: {st_next_hist.shape}')
                            with torch.no_grad():
                                self.g_net.eval()
                                # ext_state = torch.hstack([st_hist, torch.from_numpy(np.array(y_temp)).reshape(-1,1)])
                                # print(f'st_hist.unsqueeze(0).shape: {st_hist.unsqueeze(0).shape}')
                                Y_approx_all = self.g_net(st_hist.to(self.device).unsqueeze(0)).cpu().detach().numpy().reshape(-1,1)
                                ...
                                Y_approx_next_all = self.g_net(st_next_hist.to(self.device).reshape(1,1,-1)).cpu().detach().numpy().reshape(-1,1)
                    ...
                    batch.append(torch.Tensor(stack_uneven(st_hist_all, fill_value=0.)))
            ...
            if self.epoch % 1 == 0:
                if self.oversampling=='CS-SMOTE':
                    ...
                    train_batch['confidence_score'] = np.where(train_batch['confidence_score'] < 1, train_batch['confidence_score']*0.9, train_batch['confidence_score'])
    ...
    def smote(self, data_memory, confidence_score=False, ):
        ...
        if confidence_score:
            dat['confidence_score'][~df.index] = 0.99
        st = dat['state_mem']
        act = dat['action_mem']
