...
N_BOOTSTRAP = 15
class IQ_Agent:
    def __init__(self, obs_dim=None, action_dim=2, divergence='kl_fix', approx_g=True, approx_dynamics=True,
                 gamma=0.99, epsilon=0.1, q_lr=0.001,
        ...
        def L2Loss(y_hat, y):
            mse_sum = torch.nn.MSELoss(reduction='sum').to(self.device)
            return torch.sqrt(mse_sum(y_hat, y))
        ...
        if approx_dynamics:
            ...
            if approx_g:
                ...
                self.g_loss = torch.nn.L1Loss().to(self.device)
            ...
            self.env_loss = torch.nn.L1Loss().to(self.device)
           
            # self.env_loss = torch.nn.MSELoss().to(self.device)
            # self.env_loss = L2Loss
            self.env_optimizer = torch.optim.Adam(self.q_net.parameters(), lr=env_lr,)
            ...
            q_net = OfflineQNetwork_orig(obs_dim=obs_dim, action_dim=action_dim, gamma=self.gamma,
                                device=device, q_entropy=q_entropy)
            q_net.apply(q_net.weights_init_uniform)
            self.q_net = q_net.to(self.device)
            # self.env_scheduler = torch.optim.lr_scheduler.ExponentialLR(optimizer=self.env_optimizer,
                                                                    # gamma=0.9999)
        if self.classify:
            q_net = Classifier(obs_dim=obs_dim, action_dim=action_dim, gamma=self.gamma,
                                device=device, q_entropy=q_entropy)
            q_net.apply(q_net.weights_init_uniform)
            self.q_net = q_net.to(self.device)
            self.clf_loss = torch.nn.BCELoss()
    ...
    def get_rewards(self, state, y_prev):
        self.q_net.eval()
        with torch.no_grad():
            irl_q0, q0_entr = self.infer_q(state=state, action=np.zeros_like(state[:,0]))
            irl_q1, q1_entr = self.infer_q(state=state, action=np.ones_like(state[:,0]))
            irl_next_s = self.infer_next_s(state=state)
            irl_next_V = self.getV(irl_next_s, evaluate=True)
            # recover rewards
            irl_G = irl_q0
        ...
        if evaluate:
            ...
            with torch.no_grad():
                ...
                if self.q_entropy:
                    ...
                    q = torch.stack([ self.q_net.get_Q(obs.clone().to(self.device)) for i in [1]], dim=-1)
    ...
    def getQ_s_a(self, states, actions, evaluate=False, mc_estimate=False):
    ...
    def getV(self, obs, evaluate=False):
        if evaluate:
            q, q_var = self.getQ(obs, evaluate)
        else:
            q = self.getQ(obs, evaluate)
        v = self.epsilon * softmax(q/self.epsilon, dim=1, keepdim=True)
        return v
 
    def infer_q(self, state, action, evaluate=True, mc_estimate=True):
        return self.getQ_s_a(state, action, evaluate=evaluate, mc_estimate=mc_estimate)
    ...
    def iq_update(self, batch, epoch, cs, update=False,avg_entropy=False):
        ...
        if self.approx_dynamics:
            # current_Q, next_state_k = self.q_net(obs.to(self.device), action.to(self.device))
            current_Q = self.getQ(obs.to(self.device))
            next_state_k = self.getS_dash(obs.to(self.device))
            next_obs_tilde = next_state_k.clone().cpu()+obs[:,:].clone()
        else:
            # current_Q = self.q_net(obs.to(self.device), action.to(self.device))
            current_Q = self.getQ(obs.to(self.device))
           
            next_obs_tilde = next_obs[:,:].clone().cpu()
        next_V = self.getV(next_obs_tilde, evaluate=False)
 
        critic_loss, loss_dict = self.iq_loss(self.q_net, current_Q.to(self.device), current_V.to(self.device), next_V.to(self.device), batch, cs, avg_entropy=avg_entropy)
        if self.approx_dynamics:
            state_loss = self.env_loss(next_state_k.to(self.device).float(), next_obs_k[:,:].to(self.device).float()).to(self.device)
    ...
    def test(self, test_memory, from_grid=False, validation=False):
        ...
        if from_grid:
            ...
            if conservative:
                ...
                out_degree_test = np.zeros_like(state_discretised[:,0]).astype(float)
                for i in range(len(k_outs)):
                    out_degree_test[(state_discretised.astype(int)==unique_ints[i].astype(int)).all(axis=1)] =  k_outs[i].astype(float)
            ...
            G = np.full((np.unique(test_memory['path_ids']).shape[0],np.max(test_memory['time_ids']).astype(int)+1), np.nan)
            g = np.full((np.unique(test_memory['path_ids']).shape[0],np.max(test_memory['time_ids']).astype(int)+1), np.nan)
            if True:
                sample={}
                for i in range(len(mem_keys)):
                    sample[mem_keys[i]] = torch.from_numpy(np.array(test_memory[mem_keys[i]]))
                sample['state_mem'] = (sample['state_mem']-self.mean)/self.std
                # sample['state_mem'] = (sample['state_mem']-self.min)/(self.max-self.min)*(MAX-(MIN))+(MIN)
               
                # if conservative:
                #     # sample['k_out'] = np.array(list(compress(out_degree_test,test_memory['path_ids']==path_id)))
                #     sample['k_out'] = np.array(list(compress(out_degree_test,sample['path_ids'])))
                if self.classify:
                    state = sample['state_mem']    
                    Q0_results = []
                    Q1_results = []
                    G_results = []
                    g_results = []
                    ...
                    with torch.no_grad():
                        ...
                        irl_q1 = 1- irl_q0
                        # irl_q0 = self.infer_q(state, torch.zeros_like(state[:,0]))
                        # irl_q1 = self.infer_q(state, torch.ones_like(state[:,0]))
                        # q1_var = q0_var
                    irl_q0 = irl_q0.cpu()
                    irl_q1 = irl_q1.cpu()
                   
                    irl_g = torch.zeros_like(sample['state_mem'][:,0]).reshape(-1)
                   
                    for p, path_id in enumerate(np.unique(test_memory['path_ids'].astype(int))):
                        # for time_id in sample['time_ids']:
                        if conservative:
                            path_kouts = np.array(list(compress(out_degree_test,test_memory['path_ids']==path_id)))
                       
                   
                        Q0_results = np.array(irl_q0[test_memory['path_ids']==path_id])
                        Q1_results = np.array(irl_q1[test_memory['path_ids']==path_id])
                       
                        G_results = np.array(irl_q0[test_memory['path_ids']==path_id])
                        g_results = np.array(irl_g[test_memory['path_ids']==path_id])
                       
                        if conservative:
                            Q0_results[path_kouts<out_thresh] = 0
                            Q1_results[path_kouts<out_thresh] = 0
                            G_results[path_kouts<out_thresh] = 0
                            g_results[path_kouts<out_thresh] = 0
                        ...
                        Q0[p,:len(Q0_results)] = Q0_results.reshape(-1)
                        Q1[p,:len(Q1_results)] = Q1_results.reshape(-1)
                       
                        G[p,:len(G_results)] = G_results.reshape(-1)
                        g[p,:len(g_results)] = g_results.reshape(-1)
                   
                           
                else:
                    if self.approx_g:
                        irl_g = self.g_net(sample['state_mem'])
                        y_list = []
                        for p, path_id in enumerate(np.unique(test_memory['path_ids'].astype(int))):
                            # for time_id in sample['time_ids']:
                            irl_y_path = torch.cumsum(irl_g[test_memory['path_ids']==path_id], 0)
                            y_list.append(irl_y_path)
                        irl_y = torch.cat(y_list,0).flatten()
                        # print(f'irl_y.shape: {irl_y.shape}')
                        state = torch.hstack([sample['state_mem'].clone(), irl_y.reshape(-1,1)])
                        irl_g = irl_g.detach().cpu().numpy()
                    else:
                        state = sample['state_mem']    
                        # y_next_hist = []
                    # sample = sample[mem_keys[i]]
                    Q0_results = []
                    Q1_results = []
                    G_results = []
                    g_results = []
                   
                    irl_q0, q0_var = self.infer_q(state=state, action=torch.zeros_like(state[:,0]).reshape(1,-1))
                    irl_q1, q1_var = self.infer_q(state=state, action=torch.ones_like(state[:,0]).reshape(1,-1))
                    irl_q0 = irl_q0.cpu()
                    irl_q1 = irl_q1.cpu()
                    if self.approx_dynamics:
                        if not self.approx_g:
                            irl_next_s = self.infer_next_s(state)
                            irl_next_V = self.getV(irl_next_s, evaluate=True)
                            irl_g = (irl_q1 - self.gamma*irl_next_V).reshape(-1)  
                    elif not self.approx_dynamics and not self.approx_g:
                        irl_g = torch.zeros_like(sample['state_mem'][:,0]).reshape(-1)
            ...
            #                 irl_q0, q0_var = self.infer_q(state=state.reshape(1,-1), action=np.zeros((1)).reshape(1,-1))
            #                 irl_q1, q1_var = self.infer_q(state=state.reshape(1,-1), action=np.ones((1)).reshape(1,-1))
            #                 irl_q0 = irl_q0.cpu()
            #                 irl_q1 = irl_q1.cpu()
            #                 if self.approx_dynamics:
            #                     irl_next_s = self.infer_next_s(state)
            #                     irl_next_V = self.getV(irl_next_s, evaluate=True)
            #                     irl_g = irl_q1 - self.gamma*irl_next_V
            #                 else:
            #                     irl_g = torch.from_numpy(np.array([0]))
            ...
            #                     G_results.append(irl_q0.detach().cpu().numpy()[0][0])
            #                     g_results.append(irl_g.detach().cpu().numpy()[0])
            #                 # if self.approx_g:                    
            ...
            #         g_results = np.array(g_results).reshape(-1)
            ...
            # if not validation:
            # # if True:
            #     x = np.nanmedian(np.nan_to_num(Q0, nan=0),0)
            #     error_25 =np.nanquantile(np.nan_to_num(Q0, nan=0), .25, 0)
            #     error_75 = np.nanquantile(np.nan_to_num(Q0, nan=0), .75, 0)
            #     plt.plot(np.arange(len(x)),x, c='red')
            #     plt.fill_between(np.arange(len(x)), error_25, error_75, facecolor='red', alpha=0.45)
               
            #     y = np.nanmedian(np.nan_to_num(Q1, nan=0),0)
            #     y_error_25 =np.nanquantile(np.nan_to_num(Q1, nan=0), .25, 0)
            #     y_error_75 = np.nanquantile(np.nan_to_num(Q1, nan=0), .75, 0)
            #     plt.plot(np.arange(len(y)),y, c='green')
            #     plt.fill_between(np.arange(len(y)), y_error_25, y_error_75, facecolor='green', alpha=0.45)
            #     plt.title(f'Q0 vs Q1')        
            #     plt.show()
            # # if True:
            #     x = np.nanmedian(np.nan_to_num(Q0, nan=0)-np.nan_to_num(Q1, nan=0),0)
            #     error_25 =np.nanquantile(np.nan_to_num(Q0, nan=0)-np.nan_to_num(Q1, nan=0), .25, 0)
            #     error_75 = np.nanquantile(np.nan_to_num(Q0, nan=0)-np.nan_to_num(Q1, nan=0), .75, 0)
            #     plt.plot(np.arange(len(x)),x, c='red')
            #     plt.fill_between(np.arange(len(x)), error_25, error_75, facecolor='red', alpha=0.45)
            #     plt.title(f'Q0-Q1 (risk)')
            #     # if self.approx_dynamics:
            #     #     y = np.nanmedian(g,0)
            #     #     y_error_25 =np.nanquantile(g, .25, 0)
            #     #     y_error_75 = np.nanquantile(g, .75, 0)
            #     #     plt.plot(np.arange(len(y)),y, c='green')
            #     #     plt.fill_between(np.arange(len(y)), y_error_25, y_error_75, facecolor='green', alpha=0.45)
                       
            #     plt.show()
               
            #     x = np.nanmedian(np.nan_to_num(G, nan=0),0)
            #     error_25 =np.nanquantile(np.nan_to_num(G, nan=0), .25, 0)
            #     error_75 = np.nanquantile(np.nan_to_num(G, nan=0), .75, 0)
            #     plt.plot(np.arange(len(x)),x, c='red')
            #     plt.fill_between(np.arange(len(x)), error_25, error_75, facecolor='red', alpha=0.45)
               
            #     y = np.nanmedian(np.nan_to_num(g, nan=0),0)
            #     y_error_25 =np.nanquantile(np.nan_to_num(g, nan=0), .25, 0)
            #     y_error_75 = np.nanquantile(np.nan_to_num(g, nan=0), .75, 0)
            #     plt.plot(np.arange(len(y)),y, c='green')
            #     plt.fill_between(np.arange(len(y)), y_error_25, y_error_75, facecolor='green', alpha=0.45)
            #     plt.title(f'G vs g')        
            #     plt.show()
            ...
            mtte = np.mean(time_to_event)
            memr = np.mean(event_miss)
            ...
            f1 = f1_score((test_memory["action_mem"]-1)**2, (st_imitation-1)**2)
            ...
            pr_auc = average_precision_score((test_memory["action_mem"]-1)**2, (st_imitation-1)**2)
            balanced_accuracy = balanced_accuracy_score(test_memory["action_mem"], st_imitation)
            # except:
            ...
            if not validation:
                ...
                print(f'Balance Accuracy Score: {balanced_accuracy}')
                print(f'F1: {f1}')
                print(f'pr_auc: {pr_auc}')
                print(f'median time-to-event: {mtte}')
                print(f'median event miss-rate: {memr}')
                print(f'number of missed events: {np.sum(event_miss)}')
                ...
                # plt.plot(*square_coord, c="r", linewidth=2)
                # plt.show()
            return mtte, memr, f1, pr_auc, balanced_accuracy
           
    def train(self, mem, batch_size=250, n_epoches=5000, verbose=1):
        avg_entropy = self.q_entropy
        memory = mem.copy()
        ...
        if self.IS_CS:
            self.buffer_N = train_batch[0].shape[0]
        ...
        self.min = np.array(orig_train_batch['state_mem']).min(axis=0).copy()
        self.max = np.array(orig_train_batch['state_mem']).max(axis=0).copy()
        # self.mean = 0
        # self.std = 1
        counter = 0
        # for i in tqdm(range(10000)):
        for epoch in tqdm(np.arange(n_epoches)):
            self.epoch = epoch
            ...
            for b_id, ids_batch in enumerate(train_ids):
                ...
                # batch[0] = (batch[0]-self.min)/(self.max-self.min)*(MAX-(MIN))+(MIN)
                # batch[1] = (batch[1]-self.min)/(self.max-self.min)*(MAX-(MIN))+(MIN)
               
                self.q_net.train()
                if self.classify:
                    obs, next_obs, action = batch[0:3]
                    self.critic_optimizer.zero_grad()
                    outputs = self.q_net(obs)
                    loss = self.clf_loss(outputs.float(), action.reshape(-1,1).float())
                    loss.backward()
                    self.critic_optimizer.step()
                    # print(f'Epoch loss: {loss.cpu().detach()}')
                    epoch_losses+list(loss.cpu().detach().numpy().reshape(-1))
                else:  
                    if self.approx_g:
                        self.g_net.train()
                        y_hist = []
                        ...
                        for k in range(len(path_ids)):
                            ...
                            y_prev_temp = [0]
                            y_next_temp = []
                            # print(f'time_ids_hist: {time_ids_hist}')
                            for time_id in [time_ids_hist[-1]]:
                                ...
                                # st_hist = (torch.from_numpy(np.array(memory['state_mem'])[new_mask])-self.min)/(self.max-self.min)*(MAX-(MIN))+(MIN)
                                # st_next_hist = (torch.from_numpy(np.array(memory['next_state_mem'])[new_mask_next][-1,:])-self.min)/(self.max-self.min)*(MAX-(MIN))+(MIN)
                                ...
                                with torch.no_grad():
                                    ...
                                    if Y_approx_all.shape[0]>0:
                                        Y_approx_all[0,:] = 0
                                    # print(f'Y_approx_all.shape: {Y_approx_all.shape}')
                    ...
                    if self.approx_g and self.oversampling in ['LSMOTE', 'CS-LSMOTE']:
                        ...
                        # print(f'batch_split: {random_batch_split(np.arange(len(tmp_batch['time_ids'])), batch_size, stratify=tmp_batch['action_mem'])[0]}')
                        # ids_batch_second = random_batch_split(np.arange(len(tmp_batch['time_ids'])), batch_size, stratify=tmp_batch['action_mem'])
                           
                        # for b_id_new, ids_batch_new in enumerate(ids_batch_second):
                        batch = [torch.from_numpy(np.array(tmp_batch[new_mem_keys[i]])) for i in range(len(new_mem_keys))]
                        cs = torch.from_numpy(np.array(tmp_batch['confidence_score']))
                        ...
                        losses = self.iq_update(batch, epoch, cs,avg_entropy=avg_entropy)
                        epoch_losses.append(losses)
                        # batch = [torch.from_numpy(tmp_batch[new_mem_keys[i]]) for i in range(len(new_mem_keys))]
                    elif self.approx_g and self.oversampling==None:
                        self.g_net.train()
                        if b_id % 1==0:
                            g_loss = self.g_update(batch, b_id)
                        losses = self.iq_update(batch, epoch, cs,avg_entropy=avg_entropy)
            ...
            self.beta_is = np.max([self.beta_is*1.002, 1])
            self.q_net.eval()
            with torch.no_grad():
                ...
                val_balanced_accuracy = fold_balanced_accuracy
                epoch_balanced_acc = val_balanced_accuracy
                ...
                EPOCH_BALANCED_ACCURACY.append(fold_balanced_accuracy)
                # EPOCH_TRAIN_BALANCED_ACCURACY.append(train_balanced_accuracy.copy())
                EPOCH_MTTE.append(fold_mtte.copy())
                EPOCH_MEMR.append(fold_memr.copy())
                ...
                # print(f'VAL AVG MTTE: {round(fold_mtte,4)}, AVG MEMER {round(fold_memr,4)}, AVG F1 {round(fold_f1,4)}, AVG PR_AUC {round(fold_pr_auc,4)}, AVG BALANCED_ACC {round(fold_balanced_accuracy,4)}')
                counter+=1
                if self.epoch>=1:
                    if fold_balanced_accuracy>best_crossval_score:
                        # if not self.classify:
                        #     print(f'Saving best model checkpoint with balanced acc: {fold_balanced_accuracy}, avg Q-entropy: {np.mean([epoch_losses[i]["avg_entropy"] for i in range(len(epoch_losses))])}')
                        # else:
                        #     print(f'Saving best model checkpoint with balanced acc: {fold_balanced_accuracy}, avg epoch loss: {np.mean(epoch_losses)}')
                       
                        best_crossval_score = fold_balanced_accuracy
        ...
        # i f epoch % verbose==0:
            ...
            #     print(f'NEW Q LR: {param_group["lr"]}')
            # if self.approx_dynamics:
            #     for param_group in self.env_optimizer.param_groups:
            #         print(f'NEW ENV LR: {param_group["lr"]}')
            if counter>=1000000:
                break  
            if self.classify:
                EPOCH_LOSS.append(np.mean(epoch_losses))
            else:
                EPOCH_LOSS.append(np.mean([epoch_losses[i]['value_loss'] for i in range(len(epoch_losses))]))
        ...
        self.epoch_balanced_accuracy = EPOCH_BALANCED_ACCURACY
        self.epoch_mtte = EPOCH_MTTE
        self.epoch_memr = EPOCH_MEMR
        if not self.classify:
            self.epoch_loss = EPOCH_LOSS #[epoch_losses[i]['value_loss'] for i in range(len(epoch_losses))]
        else:
        ...
        # ax1.set_ylabel('Validation m-TTE', color='blue')
        # ax1.plot(EPOCH_MTTE[5:], color='blue', label='Validation m-TTE')
        # ax1.grid(False)
        # ax2 = ax1.twinx()  # instantiate a second Axes that shares the same x-axis
        # ax2.set_ylabel('Validation m-EMR', color='orange')  # we already handled the x-label with ax1
        # ax2.plot(EPOCH_MEMR[5:], color='orange', label='Validation m-EMR')
        # ax2.grid(False)
        ...
        if n_stop_act<=0:
            ...
            if self.approx_g:
                ...
                for name in df.columns:
                    unique_count = df[name].unique().shape[0]
                    if unique_count <= 5:
                        cat_cols.append(name)
                self.sm = BOOTSTRAP(
                                 sampling_strategy='minority',
                                    # shrinkage=0.001,
                                # k_neighbors=min(6, n_stop_act)
                                )
                # self.sm = SMOTENC(sampling_strategy='minority',
                #                    k_neighbors=min(6, n_stop_act),
                ...
                self.sm = SMOTE(sampling_strategy='minority',
                                k_neighbors=min(self.smote_k, n_stop_act)
                                )
                # self.sm = BOOTSTRAP(
                #                   sampling_strategy='minority',
                #                     # shrinkage=0.001,
            ...
            if self.approx_g:
                ...
                # print(f'df_res.loc[y_resampled==0,df_res.columns[-3]]: {df_res.loc[y_resampled==0,df_res.columns[-3]]}')
                # print(f'df_res.loc[y_resampled==0,df_res.columns[-4]]: {df_res.loc[y_resampled==0,df_res.columns[-4]]}')
                df_res.loc[y_resampled==0,df_res.columns[-3]] = df_res.loc[y_resampled==0,df_res.columns[-4]].values
               
            dat['state_mem'] = df_res.iloc[:,:x_size].values
            dat['next_state_mem'] = df_res.iloc[:,x_size:x_size*2].values
            if self.approx_g:
                dat['y'] = df_res.iloc[:,-4].values.reshape(-1,1)
                dat['y_next'] = df_res.iloc[:,-3].values.reshape(-1,1)
                dat['y_prev'] = df_res.iloc[:,-2].values.reshape(-1,1)
            ...
            # plt.scatter(st[~df.index][act[~df.index]==0,0],
            #             st[~df.index][act[~df.index]==0,1],
            #             s=5, c='red',
            #             label='artificial stopping decisions')
           
            # plt.scatter(data_memory['state_mem'][data_memory['action_mem']==0,0],
            #             data_memory['state_mem'][data_memory['action_mem']==0,1],s=65,
            #             c='green',label='true stopping decisions')
            # plt.legend(fontsize=25)
            # plt.xticks(fontsize=20)
            # plt.yticks(fontsize=20)
            # plt.title('Bessel(2) with radial stopping: SMOTE vs true stopping samples',fontsize=30)
            # plt.xlim((-1.5,1.5))
            # plt.ylim((-1.5,1.5))
    ...
    def iq_loss(self, agent, current_Q, current_v, next_v,
                ...
                lambda_gp=0.1, regularize=True,avg_entropy=False):
        div = self.divergence
        ...
        irl_q, q_var =self.infer_q(state=obs, action=action, mc_estimate=avg_entropy)
               
        Q_entropy = torch.mean(q_var)
        inv_var = 1.6/q_var
        loss_dict['avg_entropy'] = Q_entropy.item()
        ...
        # irl_q0_mc = torch.hstack([self.getQ_s_a(states=obs, actions=numpy.zeros_like(obs[:,0]), mc_estimate=True, evaluate=False) for i in range(25)])
        # irl_q1_mc = torch.hstack([self.getQ_s_a(states=obs, actions=numpy.ones_like(obs[:,0]), mc_estimate=True, evaluate=False) for i in range(25)])
        # print(irl_q0_mc.shape)
       
        # current_Q = torch.Tensor([torch.mean(irl_q0_mc), torch.mean(irl_q1_mc)]).reshape(-1,1)
        ...
        # print(f'current_Q: {current_Q[:5,:]}')
        # print(f'log2(current_Q): {torch.log2(current_Q[:5,:])}')
        # print(f'current_Q*log2(current_Q): {current_Q[:5,:]*torch.log2(current_Q[:5,:])}')
        # print(f'sum(current_Q*log2(current_Q)): {torch.sum(current_Q[:5,:]*torch.log2(current_Q[:5,:]),axis=1)}')
        ...
        if self.IS_CS:
            # td_err = (reward + next_v)- current_Q_s_a
            td_err = -(phi_grad * reward)
            p = torch.abs(td_err) + 0.000001
            P = (p**0.99)/(torch.sum(p**0.99))
            # w = (self.buffer_N*P)**(-self.beta_is)
            w = (P.shape[0]*P)**(-self.beta_is)
            cs = w*td_err
           
        phi_grad=phi_grad*cs
        ...
        if div == "chi":
            ...
            reward = cs*(current_Q_s_a.cpu() - rew.cpu())
        ...
        # Q_entropy = torch.sum((-1)*torch.sum(current_Q* torch.log2(current_Q),axis=1))
        ...
        # loss=loss*cs
        loss_dict['total_loss'] = loss.clone() #.item()
    ...
    def plot_st_reg_bm(self, test_memory, conservative, discretiser, out_thresh, name):
        ...
        act_mem = test_memory['action_mem']
        ...
        # my_cmap = LinearSegmentedColormap.from_list('my_cmap', cmaps, N=len(numpy.arange(state_mem[:,2].min(),state_mem[:,2].max(), 0.05))+2)
        # print(f'N=:{len(numpy.arange(state_mem[:,2].min(),state_mem[:,2].max(), 0.1))}')
        my_cmap = LinearSegmentedColormap.from_list('my_cmap', cmaps, N=len(numpy.arange(0,50, 5))+5)
        print(f'N=:{len(numpy.arange(0,50, 5))}')
       
        sns.set(font_scale = 4)
       
        # X1_test = numpy.arange(state_mem[:,0].min(),state_mem[:,0].max(), 0.1)
        # X2_test = numpy.arange(state_mem[:,1].min(),state_mem[:,1].max(), 0.1)
        # X1_test = numpy.arange(-1.5,1.5, 0.1)
        # X2_test = numpy.arange(-1.5,1.5, 0.1)
           
        # for i,j in enumerate(numpy.arange(state_mem[:,2].min(),state_mem[:,2].max(), 0.25)):
        # for i,j in enumerate(numpy.arange(0,50, 5)):
        #     j=j/50
        #     # print(f'TIME: {round(j,3)}')
        #     if self.mean.shape[0]==2:
        #         # mesh = numpy.array(numpy.meshgrid(X1_test,TIME_test)).T.reshape(-1,2)
        ...
        #         out_degree_test = np.zeros_like(state_discretised[:,0]).astype(float)
        #         for p in range(len(k_outs)):
        #             out_degree_test[(state_discretised.astype(int)==unique_ints[p].astype(int)).all(axis=1)] =  k_outs[p].astype(float)
            
        #     mesh_norm = (mesh.clone() - self.mean)/self.std
        #     # mesh_norm = (mesh.clone()-self.min)/(self.max-self.min)*(MAX-(MIN))+(MIN)
           
        #     with torch.no_grad():
        #         # print(self.infer_q(state=mesh_norm, action=numpy.zeros_like(mesh[:,0]), mc_estimate=True).shape)
        #         # irl_q0_mc, q0_var = torch.hstack([self.infer_q(state=mesh_norm, action=numpy.zeros_like(mesh[:,0]), mc_estimate=True) for i in range(25)])
        #         # irl_q1_mc, q1_var = torch.hstack([self.infer_q(state=mesh_norm, action=numpy.ones_like(mesh[:,0]), mc_estimate=True) for i in range(25)])
        #         # print(irl_q0_mc.shape)
               
               
        #         # Q0_entropy = torch.std(irl_q0_mc,axis=1)
        #         # Q1_entropy = torch.std(irl_q1_mc,axis=1)
        #         if not self.classify:
        #             irl_q0, q0_var =self.infer_q(state=mesh_norm, action=numpy.zeros_like(mesh[:,0]), mc_estimate=self.q_entropy)
        #             irl_q1, q1_var = self.infer_q(state=mesh_norm, action=numpy.ones_like(mesh[:,0]), mc_estimate=self.q_entropy)
        #             irl_curr_v = self.getV(mesh_norm, evaluate=True)
        #         else:
        #            self.q_net.eval()
        #            with torch.no_grad():
        #                irl_q0 = self.q_net(mesh_norm)
        #                irl_q1 = 1- irl_q0
        #                # q1_var = q0_var
        #            irl_q0 = irl_q0.cpu()
        #            irl_q1 = irl_q1.cpu()
        ...
        #             q0_var[out_degree_test<out_thresh] = 0.0
        #             q1_var[out_degree_test<out_thresh] = 0.0
        ...
        #             # Q_entropy = (-1)*torch.sum(irl_q0* torch.log2(irl_q0),axis=1) + (-1)*torch.sum(irl_q1* torch.log2(irl_q1),axis=1)
       
        #             # df0 = pd.DataFrame({"x1":mesh[:,0],
        #             #                 "x2":mesh[:,1],
        #             #               "Q":Q_entropy.reshape(-1)})
   
        #             # data0 = pd.pivot_table(df0, values='Q', index='x1', columns='x2')
                   
        #             # seaborn.heatmap(data0)
        #             # pyplot.title(f't={round(j,3)}, Q entropy')
        #             # pyplot.show()
        ...
        #                         "x2":mesh[:,1].reshape(-1),
        #                         "Q":irl_q0.reshape(-1)})
        #     df0.x1 = truncate(df0.x1,4)
        #     df0.x2 = truncate(df0.x2,4)
        #     # print(f'type(mesh[:,0].reshape(-1)): {type(mesh[:,0].reshape(-1))}')
        #     data0 = pd.pivot_table(df0, values='Q', index='x1', columns='x2')
        #     data_norm0 = data0
        ...
        #                     "x2":mesh[:,1].reshape(-1),
        #                     "Q":irl_q1.reshape(-1)})
        #     df1.x1 = truncate(df1.x1,4).copy()
        #     df1.x2 = truncate(df1.x2,4).copy()
            
        #     data1 = pd.pivot_table(df1, values='Q', index='x1', columns='x2')
        #     data_norm1 = data1.copy()
        #     data_diff = data_norm0.copy()-data_norm1.copy()
        ...
        #         # sns.scatterplot(boundary, ax=ax2)
        ...
        # # plt.xlim(-1.5,1.5)
        # # plt.ylim(-1.5,1.5)
        # plt.title(f'Out-thresh={self.out_thresh}', fontsize=60)
        
        # # plt.scatter(state_mem[:,0][act_mem==0], state_mem[:,1][act_mem==0], 
        # #             color='black', s=75)
        ...
        j=25/50
        # print(f'TIME: {round(j,3)}')
        # X1_test = numpy.arange(state_mem[:,0].min(),state_mem[:,0].max(), 15)
        # X2_test = numpy.arange(state_mem[:,1].min(),state_mem[:,1].max(), 100)
        X1_test = numpy.arange(-600, 600, 20)
        X2_test = numpy.arange(-9300,800, 100)
        
        TIME_test = X2_test.copy()
        # X1_test = numpy.arange(-1.5,1.5, 0.1)
        # X2_test = numpy.arange(-1.5,1.5, 0.1)
        # TIME_test = numpy.repeat(j, X1_test.shape[0])
        if self.mean.shape[0]==2:
        ...
        df0.x1 = truncate(df0.x1,2)
        df0.x2 = truncate(df0.x2,2)
        # print(f'type(mesh[:,0].reshape(-1)): {type(mesh[:,0].reshape(-1))}')
        data0 = pd.pivot_table(df0, values='Q', index='x1', columns='x2')
        data_norm0 = data0
        data_nan0 = data_norm0
        ...
        if self.q_entropy:
            ...
            df1 = pd.DataFrame({"x1":(mesh[:,0].reshape(-1)),
                        "x2":(mesh[:,1].reshape(-1)),
                        "Q":irl_q1.reshape(-1)})
        df1.x1 = truncate(df1.x1,2).copy()
        df1.x2 = truncate(df1.x2,2).copy()
        ...
        # def norm(seq):
        #     extr = max(abs(i) for i in seq)
        #     return [i/extr for i in seq]
            
        from matplotlib import colors
        norm = colors.CenteredNorm(vcenter=0)
        fig, (ax3) = plt.subplots(nrows=1, ncols=1, figsize=(70,30))
        plt.rcParams['axes.facecolor'] = 'white'
        ax3.set_xlabel('x2')
        ax3.set_ylabel('x1')
        cmaps = sns.color_palette("magma", 10)
        sns.heatmap(data0, norm=norm, ax=ax3)
        # ax3.scatter(state_mem[:,0][(act_mem==0) & (state_mem[:,-1]==j*50)], state_mem[:,1][(act_mem==0) & (state_mem[:,-1]==j*50)], color='black', s=75)
        
        ax3.set_aspect("equal", 'box')
        ax3.invert_yaxis()
        # plt.xlim(-2,2)
        # plt.ylim(-2,2)
        plt.title('Q0', fontsize=60)
        plt.tight_layout()
        plt.show()
        ...
        ax3.set_ylabel('x1')
        cmaps = sns.color_palette("magma", 10)
        sns.heatmap(data1, norm=norm, ax=ax3)
        
        # ax3.scatter(state_mem[:,0][(act_mem==0) & (state_mem[:,-1]==j)],
        #             state_mem[:,1][(act_mem==0) & (state_mem[:,-1]==j)], 
        #             color='black', s=50)
        ...
        ax3.set_ylabel('x1')
        cmaps = sns.color_palette("magma", 10)
        sns.heatmap(data0-data1, norm=norm, ax=ax3)
        
        # ax3.scatter(state_mem[:,0][act_mem==0], state_mem[:,1][act_mem==0], color='black', s=50)
        
        ax3.set_aspect("equal", 'box')
        ax3.invert_yaxis()
        
        # ax2 = ax3.twinx()
        # sns.scatterplot(x=state_mem[:,0][, 
        #                 y=state_mem[:,1],
        #                 ax=ax2)
        # ax3.set_xlim(-2,2)
        # ax3.set_ylim(-2,2)
        
        plt.title('Q0-Q1', fontsize=60)
        plt.tight_layout()
        plt.show()
        ...
        ax3.set_ylabel('x1')
        cmaps = sns.color_palette("magma", 10)
        sns.heatmap((data0-data1)<0, norm=norm, ax=ax3)
        ...
        # ax3.set_xlim(-2,2)
        # ax3.set_ylim(-2,2)
        # plt.yticks(list(data0.columns)[0::5])
        # plt.xticks(list(data0.index)[0::5])
        plt.title('Stopping region', fontsize=60)
        plt.tight_layout()
        plt.show()
        # N=10
        # def sort_points(xy: numpy.ndarray) -> numpy.ndarray:
        ...
        # # for i in range(0,9):
        # for j in numpy.arange(state_mem[:,2].min(),state_mem[:,2].max(), 0.05):
        #     try:
        #         if i==0:
        #             with open('out/states0.'+str(round(j,3))+'.npy', 'rb') as f:
        #                 states = (numpy.load(f))
        #                 # states = spherical2cartesian(states.copy())
        #             with open('out/irl_q00.'+str(round(j,3))+'.npy', 'rb') as f:
        #                 q_0 = numpy.load(f).reshape(-1)
        #             with open('out/irl_q10.'+str(round(j,3))+'.npy', 'rb') as f:
        ...
        # st_mem = (test_memory['state_mem'].copy())
        # ax2.scatter(st_mem[test_memory['action_mem']==0,0],
        #             st_mem[test_memory['action_mem']==0,1],
