from inverse_opt_stopping.q_networks import OfflineQNetwork, OfflineQNetwork_orig, gNetwork, DoubleOfflineQNetwork
import torch
...
class IQ_Agent:
    ...
    def test(self, test_memory, from_grid=False, validation=False):
        ...
        if from_grid:
            ...
            balanced_accuracy = balanced_accuracy_score((test_memory["action_mem"]-1)**2, (st_imitation-1)**2)
            # except:
            ...
            for b_id, ids_batch in enumerate(train_ids):
                ...
                if self.approx_g and self.oversampling in ['LSMOTE', 'CS-LSMOTE']:
                    ...
                    for b_id_new, ids_batch_new in enumerate(ids_batch_second):
                        batch = [torch.from_numpy(np.array([tmp_batch[new_mem_keys[i]][j] for j in ids_batch_new])) for i in range(len(new_mem_keys))]
                        cs = torch.from_numpy(np.array([tmp_batch['confidence_score'][i] for i in ids_batch_new]))
                        losses = self.iq_update(batch, epoch, cs)
                        epoch_losses.append(losses)
                        if b_id_new % 5==0:
                            self.g_net.train()
                            g_loss = self.g_update(batch, b_id_new)
