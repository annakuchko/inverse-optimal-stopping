from inverse_opt_stopping.q_networks import OfflineQNetwork, OfflineQNetwork_orig, gNetwork, DoubleOfflineQNetwork, Classifier, DegreeNetwork
import torch
import torch.nn.functional as F
import numpy as np
import numpy
from torch import logsumexp as softmax
from torch.distributions import Categorical
import pandas as pd
from matplotlib.colors import LinearSegmentedColormap
       
from scipy.ndimage.morphology import binary_dilation            
import seaborn as sns
import matplotlib.pyplot as plt
import matplotlib as mpl
from itertools import compress
from scipy.ndimage import gaussian_filter
import seaborn
import matplotlib.pyplot as pyplot
from scipy import interpolate
from sklearn.model_selection import StratifiedKFold, StratifiedShuffleSplit
import random
from sklearn.metrics import ConfusionMatrixDisplay, confusion_matrix
from matplotlib.patches import Rectangle
# from imblearn.over_sampling import SMOTE
from imblearn.over_sampling import BorderlineSMOTE as SMOTE
from imblearn.over_sampling import RandomOverSampler as BOOTSTRAP
 
from imblearn.over_sampling import SMOTENC
from sklearn.preprocessing import LabelEncoder
from copy import deepcopy
# 'js'
sns.set_style("whitegrid")
plt.rcParams['axes.facecolor'] = 'white'
import warnings
warnings.filterwarnings("ignore")
from sklearn.model_selection import train_test_split
# train, test = train_test_split(X, test_size=0.25, stratify=X['YOUR_COLUMN_LABEL'])
from tqdm import tqdm
# from torch_discounted_cumsum import discounted_cumsum_right
def truncate(n, decimals=0):
    multiplier=10**decimals
    return (n*multiplier).astype(int) / multiplier
 
def random_batch_split(indexes, batch_size, stratify=None):
    batches = []
    ids_left = indexes.copy()
    # print(f'total n of st: {sum(stratify==0)}')
    while len(ids_left)>batch_size:    
        # print(f'test_size: {batch_size/len(ids_left)}')
        try:
            _, batch_ids = train_test_split(ids_left, test_size=batch_size, stratify=stratify[ids_left])
        except:
            _, batch_ids = train_test_split(ids_left, test_size=batch_size)
        # batch_ids = random.choices(ids_left, k=batch_size)
        batches.append(batch_ids)
        # print(f'st actions in a batch: {sum(stratify[batch_ids]==0)}')
        ids_left = np.setdiff1d(ids_left, batch_ids)
    if len(list(ids_left))>1:
        batches.append(list(ids_left))
   
    return batches
MIN=0
MAX=1
N_BOOTSTRAP = 1
class IQ_Agent:
    def __init__(self, obs_dim=None, action_dim=2, divergence='kl_fix', approx_g=True, approx_dynamics=True,
                 gamma=0.99, epsilon=0.1, q_lr=0.001,
                 env_lr=0.001, g_lr=0.001,device='cpu',dt=1,
                 oversampling=None, plot_against_time=False, seed=None,
                 cross_val_splits=3, conservative=False,
                             discretiser=None,
                             out_thresh=0.005,
                             q_entropy=False,
                             SMOTE_K=12,
                             is_cs=False,
                             classify=False,
                             cs=0.999,
                             cs_decay=0.99,
                             eps_decay_rate = 0.99):
        if seed is None:
            np.random.seed()
        else:
            np.random.seed(seed)
            torch.manual_seed(seed)
        self.plot_against_time = plot_against_time
        self.gamma = gamma
        self.approx_dynamics = approx_dynamics
        self.approx_g = approx_g
        self.oversampling = oversampling
        self.device = device
        self.approx_dynamics = approx_dynamics
        self.cross_val_splits = cross_val_splits
        self.conservative = conservative
        self.discretiser=discretiser
        self.out_thresh = out_thresh
        self.lr_decay = 0.999
        self.cs = cs
        self.cs_decay = cs_decay
        self.eps_decay_rate = eps_decay_rate
        self.q_entropy = q_entropy
        self.smote_k = SMOTE_K
        self.IS_CS = is_cs
        self.beta_is = 0.4
        self.classify = classify
        
        def L2Loss(y_hat, y):
            mse_sum = torch.nn.MSELoss(reduction='sum').to(self.device)
            return torch.sqrt(mse_sum(y_hat, y))
        # if cross_val_splits!=1:
            # self.sss = StratifiedKFold(n_splits=cross_val_splits, shuffle=True)
            # self.sss = StratifiedShuffleSplit(n_splits=1, random_state=2024,
            #                          train_size=0.70, test_size=0.30)
        if obs_dim is None:
            raise ValueError(f'Specify state dimensionality')
        if approx_dynamics:
           
            if approx_g:
                g_net = gNetwork(obs_dim=obs_dim, action_dim=action_dim, gamma=self.gamma,
                                    device=device)
                g_net.apply(g_net.weights_init_uniform)
                self.g_net = g_net.to(self.device)
                print(f'g_net ON CUDA: {next(self.g_net.parameters()).is_cuda}')
                # self.g_loss = torch.nn.SmoothL1Loss().to(self.device)
                self.g_loss = torch.nn.L1Loss().to(self.device)
               
                # self.g_loss = torch.nn.MSELoss().to(self.device)
                # self.g_loss = L2Loss
                self.g_optimizer = torch.optim.Adam(self.g_net.parameters(), lr=g_lr)
                # self.g_scheduler = torch.optim.lr_scheduler.ExponentialLR(optimizer=self.g_optimizer,
                #                                                         gamma=0.9999)
                self.g_scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer=self.g_optimizer,
                                                                        mode='max', factor=self.lr_decay, patience=2,)
               
                q_net = OfflineQNetwork(obs_dim=obs_dim+1, action_dim=action_dim, gamma=self.gamma,
                                    device=device, q_entropy=q_entropy)
                # q_net = DoubleOfflineQNetwork(obs_dim=obs_dim+1, action_dim=action_dim, gamma=self.gamma,
                #                     device=device)
               
                if oversampling not in ["LSMOTE","CS-LSMOTE", None]:
                # if oversampling is not None:
                    raise ValueError(f'Cannot do {oversampling} oversampling when approximating g. Use LSMOTE.')
            else:
                q_net = OfflineQNetwork(obs_dim=obs_dim, action_dim=action_dim, gamma=self.gamma,
                                    device=device, q_entropy=q_entropy)
                # q_net = DoubleOfflineQNetwork(obs_dim=obs_dim, action_dim=action_dim, gamma=self.gamma,
                #                     device=device)
               
            q_net.apply(q_net.weights_init_uniform)
            self.q_net = q_net.to(self.device)
            # self.env_loss = torch.nn.SmoothL1Loss().to(self.device)
            self.env_loss = torch.nn.L1Loss().to(self.device)
           
            # self.env_loss = torch.nn.MSELoss().to(self.device)
            # self.env_loss = L2Loss
            self.env_optimizer = torch.optim.Adam(self.q_net.parameters(), lr=env_lr,)
            # self.env_scheduler = torch.optim.lr_scheduler.ExponentialLR(optimizer=self.env_optimizer,
                                                                    # gamma=0.9999)
           
            self.env_scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer=self.env_optimizer,
                                                                        mode='max', factor=self.lr_decay, patience=2,)
                       
        else:
            q_net = OfflineQNetwork_orig(obs_dim=obs_dim, action_dim=action_dim, gamma=self.gamma,
                                device=device, q_entropy=q_entropy)
            q_net.apply(q_net.weights_init_uniform)
            self.q_net = q_net.to(self.device)
            # self.env_scheduler = torch.optim.lr_scheduler.ExponentialLR(optimizer=self.env_optimizer,
                                                                    # gamma=0.9999)
        if self.classify:
            q_net = Classifier(obs_dim=obs_dim, action_dim=action_dim, gamma=self.gamma,
                                device=device, q_entropy=q_entropy)
            q_net.apply(q_net.weights_init_uniform)
            self.q_net = q_net.to(self.device)
            self.clf_loss = torch.nn.BCELoss()
        if self.conservative: 
            self.degree_model = DegreeNetwork()
            # self.degree_model.apply(self.degree_model.weights_init_uniform)
            self.degree_model = self.degree_model.to(self.device)
            # self.degree_loss = torch.nn.BCELoss()
            self.degree_optimizer = torch.optim.Adam(self.degree_model.parameters(), lr=0.01)
            lmbda = lambda epoch: 0.99 ** epoch
            self.degree_scheduler = torch.optim.lr_scheduler.MultiplicativeLR(self.degree_optimizer, lr_lambda=lmbda)
               
        print(f'q_net ON CUDA: {next(self.q_net.parameters()).is_cuda}')
        self.critic_optimizer = torch.optim.Adam(self.q_net.parameters(), lr=q_lr,)
        # self.scheduler = torch.optim.lr_scheduler.ExponentialLR(optimizer=self.critic_optimizer,
        #                                                         gamma=0.9999)
        self.scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer=self.critic_optimizer,
                                                                        mode='max', factor=self.lr_decay, patience=2,)
           
        self.dt = dt
        self.divergence = divergence
        self.epsilon = epsilon
        self.epoch = 0
 
    def get_rewards(self, state, y_prev, out_degree_weight=None, out_degree_weight_next=None):
        self.q_net.eval()
        with torch.no_grad():
            irl_q0, q0_entr = self.infer_q(state=state, 
                                           action=np.zeros_like(state[:,0]),
                                           out_degree_weight=out_degree_weight)
            irl_q1, q1_entr = self.infer_q(state=state, 
                                           action=np.ones_like(state[:,0]),
                                           out_degree_weight=out_degree_weight)
            
            irl_next_s = self.infer_next_s(state=state)
            if self.conservative:
                irl_next_V = self.getV(irl_next_s, 
                                       evaluate=True, 
                                       out_degree_weight=out_degree_weight_next)
            else:
                irl_next_V = self.getV(irl_next_s, 
                                       evaluate=True)
            # recover rewards
            irl_G = irl_q0
            irl_g = irl_q1 - self.q_net.gamma*irl_next_V.cpu() - y_prev
        return irl_G, irl_g
 
    def getQ(self, obs, evaluate=False,mc_estimate=False):
        if evaluate:
            self.q_net.eval()
            with torch.no_grad():
                if mc_estimate:
                    for m in self.q_net.modules():
                        if m.__class__.__name__.startswith('Dropout'):
                            m.train()
                q = torch.stack([ self.q_net.get_Q(obs.clone().to(self.device)) for i in range(N_BOOTSTRAP)], dim=-1)
                q_mean = torch.mean(q, axis=-1).reshape(-1,2)
                q_std = torch.std(q, axis=-1).reshape(-1,2)
            return q_mean.reshape(-1,2), q_std.reshape(-1,2)
        else:
            self.q_net.train()
        # with torch.no_grad():
            q = self.q_net.get_Q(obs.clone().to(self.device))
            # print(f'q.shape: {q.shape}')
            return q.reshape(-1,2)
 
    def getQ_s_a(self, states, actions, evaluate=False, mc_estimate=False, out_degree_weight=None):
        Qs, qs_var = self.getQ(states, evaluate, mc_estimate)
        if self.conservative:
            # print(f'type(Qs): {type(Qs)}')
            # print(f'type( torch.hstack([out_degree_weight, -out_degree_weight])     ): {type( torch.hstack([out_degree_weight, -out_degree_weight])     )}')
            Qs = Qs * torch.hstack([
                (1+out_degree_weight), 
                (1-out_degree_weight)
                ])     
        Q_s_a = torch.gather(Qs.cpu(), dim=-1, index=torch.tensor(actions).to(torch.int64).cpu().reshape(-1, 1))
        Q_s_a_var = torch.gather(qs_var.cpu(), dim=-1, index=torch.tensor(actions).to(torch.int64).cpu().reshape(-1, 1))
       
        return Q_s_a, Q_s_a_var
 
    def getS_dash(self, states, evaluate=False):
        if evaluate:
            self.q_net.eval()
            with torch.no_grad():
                s_dash = self.q_net.get_next_s(states.clone().to(self.device)).cpu()+states.clone()
       
        else:
            self.q_net.train()
            s_dash = self.q_net.get_next_s(states.clone().to(self.device)).cpu()+states.clone()
       
        return s_dash
 
    def getV(self, obs, evaluate=False, out_degree_weight=None):
        if evaluate:
            q, q_var = self.getQ(obs, evaluate)
        else:
            q = self.getQ(obs, evaluate)
            
        if self.conservative:
            q = q * torch.hstack([
                (1+out_degree_weight), 
                (1-out_degree_weight)])     
        v = self.epsilon * softmax(q/self.epsilon, dim=1, keepdim=True)
        return v
 
    def infer_q(self, state, action, evaluate=True, mc_estimate=True, out_degree_weight=None):
        if self.conservative:
            return self.getQ_s_a(state, action, evaluate=evaluate, mc_estimate=mc_estimate, out_degree_weight=out_degree_weight)
        else:
            return self.getQ_s_a(state, action, evaluate=evaluate, mc_estimate=mc_estimate)
 
    def infer_next_s(self, state):
        return self.getS_dash(state, evaluate=True)
 
    def g_update(self, batch, epoch):
        obs,next_obs = batch[:2]
        if self.conservative:
            unique_ints, k_outs = self.discretiser.ints
            state_discretised = self.discretiser.transform(obs.cpu().detach().numpy())
            out_degree = np.zeros_like(state_discretised[:,0]).astype(float)
            for i in range(len(k_outs)):
                out_degree[(state_discretised.astype(int)==unique_ints[i].astype(int)).all(axis=1)] =  k_outs[i].astype(float)
            out_degree_weight = self.degree_model(torch.from_numpy(out_degree).to(torch.float32))
        else:
            out_degree_weight=None
            
        if self.conservative:
            unique_ints, k_outs = self.discretiser.ints
            state_discretised_next = self.discretiser.transform(next_obs.cpu().detach().numpy())
            out_degree_next = np.zeros_like(state_discretised_next[:,0]).astype(float)
            for i in range(len(k_outs)):
                out_degree_next[(state_discretised_next.astype(int)==unique_ints[i].astype(int)).all(axis=1)] =  k_outs[i].astype(float)
            out_degree_weight_next = self.degree_model(torch.from_numpy(out_degree_next).to(torch.float32))
        else:
            out_degree_weight_next=None
        y, y_next, y_prev = batch[-3:]
        obs_ext = torch.hstack([obs, y]).clone()
        # obs_ext_prev = torch.hstack([obs, y_prev]).clone()
        self.g_net.train()
       
        # g_phi = self.g_net(obs_ext_prev.to(self.device))
        g_phi = self.g_net(obs.to(self.device))
        g_iq = self.get_rewards(obs_ext,y_prev, 
                                out_degree_weight=out_degree_weight,
                                out_degree_weight_next=out_degree_weight_next)[1]
        # print()
        g_loss = self.g_loss(g_phi.to(self.device).float(), g_iq.to(self.device).float()).to(self.device)
        # if epoch %1==0:
        self.g_optimizer.zero_grad()
        g_loss.backward()
        self.g_optimizer.step()
           
           
        return g_loss.detach().cpu().numpy()
 
    def iq_update(self, batch, epoch, cs, update=False,avg_entropy=False):
        # expert_obs, expert_next_obs, expert_action, expert_reward, expert_done = expert_batch
        obs, next_obs, action = batch[0:3]
        if self.conservative:
            unique_ints, k_outs = self.discretiser.ints
            state_discretised = self.discretiser.transform(obs.cpu().detach().numpy())
            out_degree = np.zeros_like(state_discretised[:,0]).astype(float)
            for i in range(len(k_outs)):
                out_degree[(state_discretised.astype(int)==unique_ints[i].astype(int)).all(axis=1)] =  k_outs[i].astype(float)
            out_degree_weight = self.degree_model(torch.from_numpy(out_degree).to(torch.float32))
            
            # state_discretised_next = self.discretiser.transform(next_obs.cpu().detach().numpy())
            # out_degree_next = np.zeros_like(state_discretised_next[:,0]).astype(float)
            # for i in range(len(k_outs)):
            #     out_degree_next[(state_discretised_next.astype(int)==unique_ints[i].astype(int)).all(axis=1)] =  k_outs[i].astype(float)
            # out_degree_weight_next = self.degree_model(torch.from_numpy(out_degree_next).to(torch.float32))
        
        else:
            out_degree_weight=None
            # out_degree_weight_next=None
        if self.approx_g:
            y,y_next,y_prev = batch[-3:]
            obs = torch.hstack([obs, y]).clone()
            next_obs = torch.hstack([next_obs, y_next]).clone()
 
        next_obs_k = next_obs-obs
        if self.approx_g:
            self.g_net.train()
 
        self.q_net.train()
 
        if self.conservative:
            current_V = self.getV(obs, 
                                  evaluate=False, 
                                  out_degree_weight=(out_degree_weight.clone().cpu().detach()))
        
        else:
            current_V = self.getV(obs, evaluate=False)
        
        if self.approx_dynamics:
            # current_Q, next_state_k = self.q_net(obs.to(self.device), action.to(self.device))
            current_Q = self.getQ(obs.to(self.device))
            next_state_k = self.getS_dash(obs.to(self.device))
            next_obs_tilde = next_state_k.clone().cpu()+obs[:,:].clone()
        else:
            # current_Q = self.q_net(obs.to(self.device), action.to(self.device))
            current_Q = self.getQ(obs.to(self.device))
           
            next_obs_tilde = next_obs[:,:].clone().cpu()
            
        if self.conservative:
            # print(f'next_obs_tilde.shape: {next_obs_tilde.shape}')
            if self.approx_g:
                state_discretised_next = self.discretiser.transform(next_obs_tilde[:,:-1].cpu().detach().numpy())
            else:
                state_discretised_next = self.discretiser.transform(next_obs_tilde.cpu().detach().numpy())
            out_degree_next = np.zeros_like(state_discretised_next[:,0]).astype(float)
            for i in range(len(k_outs)):
                out_degree_next[(state_discretised_next.astype(int)==unique_ints[i].astype(int)).all(axis=1)] =  k_outs[i].astype(float)
            out_degree_weight_next = self.degree_model(torch.from_numpy(out_degree_next).to(torch.float32))
            
            next_V = self.getV(next_obs_tilde, 
                               evaluate=False, 
                               out_degree_weight=out_degree_weight_next.clone().cpu().detach())
        else:
            next_V = self.getV(next_obs_tilde, evaluate=False)
            out_degree_weight_next=None
        critic_loss, loss_dict = self.iq_loss(self.q_net,
                                              current_Q.to(self.device), 
                                              current_V.to(self.device), 
                                              next_V.to(self.device), 
                                              batch, 
                                              cs, 
                                              avg_entropy=avg_entropy,
                                              out_degree_weight=out_degree_weight)
        if self.approx_dynamics:
            state_loss = self.env_loss(next_state_k.to(self.device).float(), next_obs_k[:,:].to(self.device).float()).to(self.device)
            loss_dict['env_loss'] = state_loss.detach().cpu().numpy()
        # Optimize the critic
        if self.conservative:
            self.degree_optimizer.zero_grad()
        self.critic_optimizer.zero_grad()
        if self.approx_dynamics:
            self.env_optimizer.zero_grad()
        
            state_loss.backward(retain_graph=True, create_graph=True)
 
        critic_loss.backward()
        # step critic
        if self.approx_dynamics:
            if epoch % 1==0:
                self.env_optimizer.step()
        self.critic_optimizer.step()
        if self.conservative:
            if epoch % 5==0:
                self.degree_optimizer.step()
        
        return loss_dict
 
    def choose_action(self, state, sample=False):
        if isinstance(state):
            state = np.array(state)
        state = torch.FloatTensor(state).to(self.device).unsqueeze(0)
        with torch.no_grad():
            q, next_state_k = self.q_net(state.to(self.device))
            dist = F.softmax(q/self.epsilon, dim=1)
            dist = Categorical(dist)
            action = torch.argmax(dist, dim=1)
 
        return action.detach().cpu().numpy()[0]
 
    def test(self, test_memory, from_grid=False, validation=False):
        if not validation:
            self.q_net.load_state_dict(self.best_q_net)
            if self.approx_g:
                self.g_net.load_state_dict(self.best_g_net)
        self.q_net.eval()
        if self.approx_g:
            self.g_net.eval()
        conservative=self.conservative
        discretiser=self.discretiser
        out_thresh=self.out_thresh
       
        if from_grid:
            self.plot_st_reg_bm(test_memory, conservative, discretiser, out_thresh, name=f'Out-degeee threshold={out_thresh}')
        else:
            if conservative:
                unique_ints, k_outs = discretiser.ints
                state_discretised = discretiser.transform(test_memory['state_mem'])
                out_degree_test = np.zeros_like(state_discretised[:,0]).astype(float)
                for i in range(len(k_outs)):
                    out_degree_test[(state_discretised.astype(int)==unique_ints[i].astype(int)).all(axis=1)] =  k_outs[i].astype(float)
                with torch.no_grad():
                    out_degree_weight = self.degree_model(torch.from_numpy(out_degree_test).to(torch.float32)).cpu().detach().numpy()
            else:
                out_degree_weight=None
            mem_keys = list(test_memory.keys())
            Q0 = np.full((np.unique(test_memory['path_ids']).shape[0],np.max(test_memory['time_ids']).astype(int)+1), np.nan)
            Q1 = np.full((np.unique(test_memory['path_ids']).shape[0],np.max(test_memory['time_ids']).astype(int)+1), np.nan)
            G = np.full((np.unique(test_memory['path_ids']).shape[0],np.max(test_memory['time_ids']).astype(int)+1), np.nan)
            g = np.full((np.unique(test_memory['path_ids']).shape[0],np.max(test_memory['time_ids']).astype(int)+1), np.nan)
            if True:
                sample={}
                for i in range(len(mem_keys)):
                    sample[mem_keys[i]] = torch.from_numpy(np.array(test_memory[mem_keys[i]]))
                sample['state_mem'] = (sample['state_mem']-self.mean)/self.std
                
                if self.classify:
                    state = sample['state_mem']    
                    Q0_results = []
                    Q1_results = []
                    G_results = []
                    g_results = []
                   
                    self.q_net.eval()
                    with torch.no_grad():
                        # irl_q0 = self.q_net(state)
                        # irl_q1 = 1- irl_q0
                        irl_q0 = self.infer_q(state, 0)
                        irl_q1 = self.infer_q(state, 1)
                        # q1_var = q0_var
                    irl_q0 = irl_q0.cpu()
                    irl_q1 = irl_q1.cpu()
                   
                    irl_g = torch.zeros_like(sample['state_mem'][:,0]).reshape(-1)
                   
                    for p, path_id in enumerate(np.unique(test_memory['path_ids'].astype(int))):
                        if conservative:
                            path_kouts = np.array(list(compress(out_degree_test,test_memory['path_ids']==path_id)))
                       
                   
                        Q0_results = np.array(irl_q0[test_memory['path_ids']==path_id])
                        Q1_results = np.array(irl_q1[test_memory['path_ids']==path_id])
                       
                        G_results = np.array(irl_q0[test_memory['path_ids']==path_id])
                        g_results = np.array(irl_g[test_memory['path_ids']==path_id])
                       
                        if conservative:
                            Q0_results = (1+out_degree_weight[test_memory['path_ids']==path_id])*Q0_results
                            Q1_results = (1-out_degree_weight[test_memory['path_ids']==path_id])*Q1_results

                        st_id = np.where(Q0_results>=Q1_results)[0]
                        if st_id.size!=0:
                            if isinstance(st_id, np.ndarray):
                                st_id = st_id[0]
                            Q0_results[st_id+1:] = 0
                            Q1_results[st_id+1:] = 0
                            G_results[st_id+1:] = 0
                            g_results[st_id+1:] = 0
                       
                        Q0[p,:len(Q0_results)] = Q0_results.reshape(-1)
                        Q1[p,:len(Q1_results)] = Q1_results.reshape(-1)
                       
                        G[p,:len(G_results)] = G_results.reshape(-1)
                        g[p,:len(g_results)] = g_results.reshape(-1)
                   
                           
                else:
                    if self.approx_g:
                        irl_g = self.g_net(sample['state_mem'])
                        y_list = []
                        for p, path_id in enumerate(np.unique(test_memory['path_ids'].astype(int))):
                            # for time_id in sample['time_ids']:
                            irl_y_path = torch.cumsum(irl_g[test_memory['path_ids']==path_id], 0)
                            y_list.append(irl_y_path)
                        irl_y = torch.cat(y_list,0).flatten()
                        # print(f'irl_y.shape: {irl_y.shape}')
                        state = torch.hstack([sample['state_mem'].clone(), irl_y.reshape(-1,1)])
                        irl_g = irl_g.detach().cpu().numpy()
                    else:
                        state = sample['state_mem']    
                        # y_next_hist = []
                    # sample = sample[mem_keys[i]]
                    Q0_results = []
                    Q1_results = []
                    G_results = []
                    g_results = []
                   
                    irl_q0, q0_var = self.infer_q(state=state, 
                                                  action=torch.zeros_like(state[:,0]).reshape(1,-1),
                                                  out_degree_weight=torch.from_numpy(out_degree_weight))
                    irl_q1, q1_var = self.infer_q(state=state, 
                                                  action=torch.ones_like(state[:,0]).reshape(1,-1),
                                                  out_degree_weight=torch.from_numpy(out_degree_weight))
                    irl_q0 = irl_q0.cpu()
                    irl_q1 = irl_q1.cpu()
                    if self.approx_dynamics:
                        if not self.approx_g:
                            irl_next_s = self.infer_next_s(state)
                            if conservative:
                                unique_ints, k_outs = discretiser.ints
                                state_discretised_next = discretiser.transform(irl_next_s.cpu().detach().numpy())
                                out_degree_test_next = np.zeros_like(state_discretised[:,0]).astype(float)
                                for i in range(len(k_outs)):
                                    out_degree_test_next[(state_discretised_next.astype(int)==unique_ints[i].astype(int)).all(axis=1)] =  k_outs[i].astype(float)
                                with torch.no_grad():
                                    out_degree_weight_next = self.degree_model(torch.from_numpy(out_degree_test_next).to(torch.float32)).cpu().detach().numpy()
                            else:
                                out_degree_weight_next=None
                            irl_next_V = self.getV(irl_next_s, 
                                                   evaluate=True, 
                                                   out_degree_weight=torch.from_numpy(out_degree_weight_next))
                            irl_g = (irl_q1 - self.gamma*irl_next_V).reshape(-1)  
                    elif not self.approx_dynamics and not self.approx_g:
                        irl_g = torch.zeros_like(sample['state_mem'][:,0]).reshape(-1)
                   
                    for p, path_id in enumerate(np.unique(test_memory['path_ids'].astype(int))):
                        # for time_id in sample['time_ids']:
                        if conservative:
                            path_kouts = np.array(list(compress(out_degree_test,test_memory['path_ids']==path_id)))
                       
                   
                        Q0_results = np.array(irl_q0[test_memory['path_ids']==path_id])
                        Q1_results = np.array(irl_q1[test_memory['path_ids']==path_id])
                       
                        G_results = np.array(irl_q0[test_memory['path_ids']==path_id])
                        g_results = np.array(irl_g[test_memory['path_ids']==path_id])
                       
                        # if self.conservative:
                        #     Q0_results = (out_degree_weight[test_memory['path_ids']==path_id]) + Q0_results
                        #     Q1_results= (-out_degree_weight[test_memory['path_ids']==path_id]) + Q1_results
                        
                        st_id = np.where(Q0_results>=Q1_results)[0]
                        if st_id.size!=0:
                            if isinstance(st_id, np.ndarray):
                                st_id = st_id[0]
                            Q0_results[st_id+1:] = 0
                            Q1_results[st_id+1:] = 0
                            G_results[st_id+1:] = 0
                            g_results[st_id+1:] = 0
                       
                        Q0[p,:len(Q0_results)] = Q0_results.reshape(-1)
                        Q1[p,:len(Q1_results)] = Q1_results.reshape(-1)
                       
                        G[p,:len(G_results)] = G_results.reshape(-1)
                        g[p,:len(g_results)] = g_results.reshape(-1)
                   
 
           
            Q_diff = Q0.copy()-Q1.copy()
            st = Q_diff.copy()
            st[st>=0]=0
            st[st<0]=1
           
            firs_st = np.array([np.where(st[i,:]==0,1,np.nan)[0] for i in range(st.shape[0])])
            idxrow,idxcol=np.indices(st.shape)
            firs_st2=firs_st[:,None]
            mask = idxcol > firs_st2
            st[mask]=0
            st = st.flatten()
            n_paths = len(np.unique(test_memory['path_ids']))
            n_times = len(np.unique(test_memory['time_ids']))
            from sklearn.metrics import roc_auc_score
            
            st_imitation = pd.DataFrame({
                'path_ids': np.array([np.repeat(i,st.shape[0]/len(np.unique(test_memory['path_ids']))) for i in np.unique(test_memory['path_ids'])]).reshape(-1),
                'time_ids': np.array([np.arange(
                    (test_memory['time_ids']).astype(int).min(),
                     (test_memory['time_ids']).astype(int).max()+1
                     ) for i in range(n_paths)]).reshape(-1),
                'st_region':st
                })
            # print(st_imitation)
            test_ids = [(test_memory['path_ids'][i], test_memory['time_ids'][i]) for i in range(len(test_memory['path_ids']))]
            ids_to_keep = []
            # don't need this filtering for proper test paths, since they should not be stopped
            for i in range(st.shape[0]):
                if (st_imitation.path_ids[i],st_imitation.time_ids[i]) in test_ids:
                    ids_to_keep.append(i)
            st_imitation = st_imitation.iloc[ids_to_keep]
            time_to_event = []
            event_miss = []
            for p, path in enumerate(np.unique(test_memory['path_ids'])):
                path_data = st_imitation[st_imitation.path_ids==path]
                path_expert = np.array([test_memory[i] for i in list(['time_ids','action_mem'])])[:,test_memory['path_ids']==path]
                expert_st_time = path_expert[0,:][path_expert[1,:]==0]
                if expert_st_time.size>0:
                    expert_st_time = expert_st_time[0]
                else:
                    expert_st_time = np.nan
                if (path_data.st_region==0).sum()==0:
                    imitation_st_time = np.array([np.nan])
                else:
                    imitation_st_time = path_data[path_data.st_region==0].time_ids.values[0]
                # print(f'imitation_st_time: {imitation_st_time}')
                # print(f'expert st time: {expert_st_time}')
                if np.isnan(imitation_st_time).sum()==1:
                    event_miss.append(1)
                elif np.isnan(expert_st_time):
                    event_miss.append(0)
                    time_to_event.append(path_expert[0,:].max()-imitation_st_time)  
                else:
                    if int(expert_st_time)<int(imitation_st_time):
                        event_miss.append(1)
                    else:
                        event_miss.append(0)
                        time_to_event.append(expert_st_time-imitation_st_time)
                       
           
            mtte = np.mean(time_to_event)
            memr = np.mean(event_miss)
           
            st_imitation = st[ids_to_keep]
            st_imitation[np.isnan(st_imitation)] = 0
            from sklearn.metrics import roc_auc_score, accuracy_score, precision_score, recall_score, f1_score, average_precision_score, balanced_accuracy_score
            
            f1 = f1_score((test_memory["action_mem"]-1)**2, (st_imitation-1)**2)
            pr_auc = average_precision_score((test_memory["action_mem"]-1)**2, (st_imitation-1)**2)
            balanced_accuracy = balanced_accuracy_score(test_memory["action_mem"], st_imitation)
            if not validation:
                self.mtte = mtte
                self.memr = memr
                self.f1 = f1
                self.pr_auc = pr_auc
                self.balanced_accuracy = balanced_accuracy
                print(f'Balance Accuracy Score: {balanced_accuracy}')
                print(f'F1: {f1}')
                print(f'pr_auc: {pr_auc}')
                print(f'median time-to-event: {mtte}')
                print(f'median event miss-rate: {memr}')
                print(f'number of missed events: {np.sum(event_miss)}')
           
               
            return mtte, memr, f1, pr_auc, balanced_accuracy
           
    def train(self, mem, batch_size=250, n_epoches=5000, verbose=1):
        avg_entropy = self.q_entropy
        memory = mem.copy()
        memory['confidence_score'] = np.ones_like(memory['action_mem'])
        EPOCH_BALANCED_ACCURACY = []
        EPOCH_TRAIN_BALANCED_ACCURACY = []
        EPOCH_MTTE = []
        EPOCH_MEMR = []
        EPOCH_LOSS = []
        best_crossval_score = -np.inf
        mem_keys = list(memory.keys())
        unique_paths = np.unique(memory['path_ids'])
        train_path_ids = random.sample(list(unique_paths), round(len(unique_paths)*0.70))
        train_path_ids = [int(i) for i in train_path_ids]
        val_path_ids = list(set(unique_paths) - set(train_path_ids))
        val_path_ids = [int(i) for i in val_path_ids]
        all_ids = np.arange(len(memory['path_ids']))
        mask_train = np.isin(memory['path_ids'], train_path_ids)
        mask_val = np.isin(memory['path_ids'], val_path_ids)
        train_index = all_ids[mask_train]
        val_index = all_ids[mask_val]
       
        train_batch = [np.array([memory[mem_keys[l]][m] for m in train_index]) for l in range(len(mem_keys))]
       
        orig_train_batch = dict(zip(mem_keys, train_batch))
        if self.IS_CS:
            self.buffer_N = train_batch[0].shape[0]
        if self.oversampling=='SMOTE' or self.oversampling=='CS-SMOTE':
            smote_fit_batch = dict(zip(mem_keys, train_batch))    
            if self.oversampling=='CS-SMOTE':
                cf=True
            else:
                cf=False
            train_batch = self.smote(smote_fit_batch.copy(),confidence_score=cf)
        else:
            train_batch = dict(zip(mem_keys, train_batch))
        self.mean = np.array(orig_train_batch['state_mem']).mean(axis=0).copy()
        self.std = np.array(orig_train_batch['state_mem']).std(axis=0).copy()
       
        self.min = np.array(orig_train_batch['state_mem']).min(axis=0).copy()
        self.max = np.array(orig_train_batch['state_mem']).max(axis=0).copy()
        # self.mean = 0
        # self.std = 1
        counter = 0
        # for i in tqdm(range(10000)):
        for epoch in tqdm(np.arange(n_epoches)):
            self.epoch = epoch
           
            val_mtte = None
            val_memr = None
            val_f1 = None
            val_pr_auc = None
            val_balanced_accuracy = None
            epoch_losses = []
           
            train_ids = random_batch_split(np.arange(len(train_batch['time_ids'])), batch_size, stratify=train_batch['action_mem'])
            mem_keys = list(memory.keys())
            for b_id, ids_batch in enumerate(train_ids):
                path_ids = np.array([train_batch['path_ids'][i] for i in ids_batch])
                time_ids = np.array([train_batch['time_ids'][i] for i in ids_batch])
                cs = torch.from_numpy(np.array([train_batch['confidence_score'][i] for i in ids_batch]))
                batch = [torch.from_numpy(np.array([train_batch[mem_keys[i]][j] for j in ids_batch])) for i in range(len(mem_keys))]
                batch[0] = (batch[0]-self.mean)/self.std
                batch[1] = (batch[1]-self.mean)/self.std
               
                self.q_net.train()
                if self.classify:
                    obs, next_obs, action = batch[0:3]
                    self.critic_optimizer.zero_grad()
                    outputs = self.q_net(obs)
                    loss = self.clf_loss(outputs.float(), action.reshape(-1,1).float())
                    loss.backward()
                    self.critic_optimizer.step()
                    # print(f'Epoch loss: {loss.cpu().detach()}')
                    epoch_losses+list(loss.cpu().detach().numpy().reshape(-1))
                else:  
                    if self.approx_g:
                        self.g_net.train()
                        y_hist = []
                        y_hist_prev = []
                        y_next_hist = []
                        for k in range(len(path_ids)):
                            mask = (memory['path_ids']==path_ids[k]) & (memory['time_ids']<=time_ids[k])
                            # mask_next = (memory['path_ids']==path_ids[k]) & (memory['time_ids']<=time_ids[k]+1)
                            # st_hist = (torch.from_numpy(np.array(memory['state_mem'])[mask])-self.mean)/self.std
                            # st_next_hist = (torch.from_numpy(np.array(memory['next_state_mem'])[mask_next][-1,:])-self.mean)/self.std
                            time_ids_hist = np.array(memory['time_ids'])[mask]
                            y_temp = [0]
                            y_prev_temp = [0]
                            y_next_temp = []
                            # print(f'time_ids_hist: {time_ids_hist}')
                            for time_id in [time_ids_hist[-1]]:
                                new_mask = (memory['path_ids']==path_ids[k]) & (memory['time_ids']<=time_id)
                                new_mask_next = (memory['path_ids']==path_ids[k]) & (memory['time_ids']<=time_id+1)
                                st_hist = (torch.from_numpy(np.array(memory['state_mem'])[new_mask])-self.mean)/self.std
                                st_next_hist = (torch.from_numpy(np.array(memory['next_state_mem'])[new_mask_next][-1,:])-self.mean)/self.std
                               
                                # st_hist = (torch.from_numpy(np.array(memory['state_mem'])[new_mask])-self.min)/(self.max-self.min)*(MAX-(MIN))+(MIN)
                                # st_next_hist = (torch.from_numpy(np.array(memory['next_state_mem'])[new_mask_next][-1,:])-self.min)/(self.max-self.min)*(MAX-(MIN))+(MIN)
                               
                                with torch.no_grad():
                                    self.g_net.eval()
                                    # ext_state = torch.hstack([st_hist, torch.from_numpy(np.array(y_temp)).reshape(-1,1)])
                                    Y_approx_all = self.g_net(st_hist.to(self.device)).cpu().detach().numpy().reshape(-1,1)
                                    if Y_approx_all.shape[0]>0:
                                        Y_approx_all[0,:] = 0
                                    # print(f'Y_approx_all.shape: {Y_approx_all.shape}')
                                    Y_approx = (Y_approx_all*(self.gamma**(np.arange(Y_approx_all.shape[0]))).reshape(-1,1)).sum()*self.dt
                                   
               
                                    Y_approx_prev = (Y_approx_all[:-1]*(self.gamma**(np.arange(Y_approx_all[:-1].shape[0]))).reshape(-1,1)).sum()*self.dt
                                    y_prev_temp.append(Y_approx_prev)
                                    # y_hist_prev.append(Y_approx_prev)
                                   
                                    # ext_st_next = torch.hstack([st_next_hist.reshape(1,-1), torch.from_numpy(np.array(y_temp[-1])).reshape(-1,1)])
                                    Y_approx_next_all = self.g_net(st_next_hist.to(self.device)).cpu().detach().numpy().reshape(-1,1)
                                    if Y_approx_next_all.shape[0]==1:
                                        Y_approx_all[0,:] = 0
                                    Y_approx_next = (Y_approx_next_all*(self.gamma**(Y_approx_all.shape[0]+1))).sum()*self.dt + Y_approx
                                    y_next_temp.append(Y_approx_next)
                           
                            y_hist.append(y_temp[-1])
                            y_hist_prev.append(y_prev_temp[-1])
                            y_next_hist.append(y_next_temp[-1])
                           
   
                        batch.append(torch.Tensor(np.array(y_hist).reshape(-1,1)))
                        batch.append(torch.Tensor(np.array(y_next_hist).reshape(-1,1)))
                        batch.append(torch.Tensor(np.array(y_hist_prev).reshape(-1,1)))
                    if self.approx_g and self.oversampling in ['LSMOTE', 'CS-LSMOTE']:
                        # if self.oversampling=='SMOTE' or self.oversampling=='CS-SMOTE':
                        tmp_batch = batch
                        new_mem_keys = mem_keys + ['y', 'y_next', 'y_prev']
                        smote_fit_batch = dict(zip(new_mem_keys, tmp_batch))    
                        if self.oversampling=='CS-LSMOTE':
                            cf=True
                        else:
                            cf=False
                        # print(f'N stop actions in a batch: {(smote_fit_batch['action_mem']==0).sum()}')
                        tmp_batch = self.smote(smote_fit_batch.copy(),confidence_score=cf)
                        # print(f'batch_split: {random_batch_split(np.arange(len(tmp_batch['time_ids'])), batch_size, stratify=tmp_batch['action_mem'])[0]}')
                        # ids_batch_second = random_batch_split(np.arange(len(tmp_batch['time_ids'])), batch_size, stratify=tmp_batch['action_mem'])
                           
                        # for b_id_new, ids_batch_new in enumerate(ids_batch_second):
                        batch = [torch.from_numpy(np.array(tmp_batch[new_mem_keys[i]])) for i in range(len(new_mem_keys))]
                        cs = torch.from_numpy(np.array(tmp_batch['confidence_score']))
                        #     losses = self.iq_update(batch, epoch, cs)
                        #     epoch_losses.append(losses)
                        #     if b_id_new % 1==0:
                        self.g_net.train()
                        if b_id % 1==0:
                            g_loss = self.g_update(batch, b_id)
                        losses = self.iq_update(batch, epoch, cs,avg_entropy=avg_entropy)
                        epoch_losses.append(losses)
                        # batch = [torch.from_numpy(tmp_batch[new_mem_keys[i]]) for i in range(len(new_mem_keys))]
                    elif self.approx_g and self.oversampling==None:
                        self.g_net.train()
                        if b_id % 1==0:
                            g_loss = self.g_update(batch, b_id)
                        losses = self.iq_update(batch, epoch, cs,avg_entropy=avg_entropy)
                        epoch_losses.append(losses)
                       
                    else:
                        losses = self.iq_update(batch, epoch, cs,avg_entropy=avg_entropy)
                        epoch_losses.append(losses)
                   
                   
               
            # if self.epoch % 1 == 0:
            self.epsilon = self.epsilon*self.eps_decay_rate
            if self.oversampling=='CS-LSMOTE':
                self.cs = self.cs*self.cs_decay
                # print(f'New confidence score: {self.cs}')
           
            if self.oversampling=='CS-SMOTE':
                # train_batch['confidence_score'][] = train_batch['confidence_score']*0.9
                train_batch['confidence_score'] = np.where(train_batch['confidence_score'] < 1, train_batch['confidence_score']*self.cs_decay, train_batch['confidence_score'])
                # print(f'New confidence score: {train_batch['confidence_score'][train_batch['confidence_score']!=1][0]}')
            self.beta_is = np.max([self.beta_is*1.002, 1])
            self.q_net.eval()
            with torch.no_grad():
                path_ids_val = np.array([memory['path_ids'][i] for i in val_index])
                time_ids_val = np.array([memory['time_ids'][i] for i in val_index])
               
                if self.approx_g:
                    self.g_net.eval()
                    batch_val = [np.array([memory[mem_keys[m]][n] for n in val_index]) for m in range(len(mem_keys))]
                else:
                    batch_val = [np.array([memory[mem_keys[m]][n] for n in val_index]) for m in range(len(mem_keys))]
                batch_val = dict(zip(mem_keys, batch_val))
                # print(f'batch_val["path_ids"]: {batch_val["path_ids"]}')
                # print(f'batch_val["time_ids"]: {batch_val["time_ids"]}')
                fold_mtte, fold_memr, fold_f1, fold_pr_auc, fold_balanced_accuracy = self.test(test_memory=batch_val, validation=True)
                # train_mtte, train_memr, train_f1, train_pr_auc, train_balanced_accuracy  = self.test(test_memory=orig_train_batch, validation=True)
               
                val_mtte = fold_mtte.copy()
                val_memr = fold_memr.copy()
                val_f1 = fold_f1
                val_pr_auc = fold_pr_auc
                val_balanced_accuracy = fold_balanced_accuracy
                epoch_balanced_acc = val_balanced_accuracy
               
                if self.approx_g:
                    self.g_scheduler.step(fold_balanced_accuracy)
                self.scheduler.step(fold_balanced_accuracy)
                if self.conservative:
                    self.degree_scheduler.step()
                if self.approx_dynamics:
                    self.env_scheduler.step(fold_balanced_accuracy)
                EPOCH_BALANCED_ACCURACY.append(fold_balanced_accuracy)
                # EPOCH_TRAIN_BALANCED_ACCURACY.append(train_balanced_accuracy.copy())
                EPOCH_MTTE.append(fold_mtte.copy())
                EPOCH_MEMR.append(fold_memr.copy())
               
                # print(f'EPOCH: {epoch}, # cross validation splits: {self.cross_val_splits}')
                # print(f'AVG MTTE: {round(np.mean(val_mtte),4)}, AVG MEMER {round(np.mean(val_memr),4)}, AVG F1 {round(np.mean(val_f1),4)}, AVG PR_AUC {round(np.mean(val_pr_auc),4)}, AVG BALANCED_ACC {round(np.mean(val_balanced_accuracy),4)}')
               
                # print("------------------------------------")                
                # print(f'TRAIN AVG MTTE: {round(train_mtte,4)}, AVG MEMER {round(train_memr,4)}, AVG F1 {round(train_f1,4)}, AVG PR_AUC {round(train_pr_auc,4)}, AVG BALANCED_ACC {round(train_balanced_accuracy,4)}')
                # print(f'VAL AVG MTTE: {round(fold_mtte,4)}, AVG MEMER {round(fold_memr,4)}, AVG F1 {round(fold_f1,4)}, AVG PR_AUC {round(fold_pr_auc,4)}, AVG BALANCED_ACC {round(fold_balanced_accuracy,4)}')
                counter+=1
                if self.epoch>=1:
                    if fold_balanced_accuracy>best_crossval_score:
                        # if not self.classify:
                        #     print(f'Saving best model checkpoint with balanced acc: {fold_balanced_accuracy}, avg Q-entropy: {np.mean([epoch_losses[i]["avg_entropy"] for i in range(len(epoch_losses))])}')
                        # else:
                        #     print(f'Saving best model checkpoint with balanced acc: {fold_balanced_accuracy}, avg epoch loss: {np.mean(epoch_losses)}')
                       
                        best_crossval_score = fold_balanced_accuracy
                        self.best_q_net = deepcopy(self.q_net.state_dict())
                        counter = 0
                        # torch.save(self.q_net.state_dict(), 'best_q_net.pt')
                        if self.approx_g:
                            self.best_g_net = deepcopy(self.g_net.state_dict())
                            # torch.save(self.g_net .state_dict(), 'best_g_net.pt')
                       
                val_mtte = None
                val_memr = None
                val_f1 = None
                val_pr_auc = None
                val_balanced_accuracy = None
               
               
        # i f epoch % verbose==0:
            #     # epoch_avg_loss = es
            #     if self.approx_g:
            #         print(f'{epoch} epoch, loss: {losses}, g_loss: {g_loss}')
            #     else:
            #         print(f'{epoch} epoch, loss: {losses}')
 
            # for param_group in self.critic_optimizer.param_groups:
            #     print(f'NEW Q LR: {param_group["lr"]}')
            # if self.approx_dynamics:
            #     for param_group in self.env_optimizer.param_groups:
            #         print(f'NEW ENV LR: {param_group["lr"]}')
            if counter>=1000000:
                break  
            if self.classify:
                EPOCH_LOSS.append(np.mean(epoch_losses))
            else:
                EPOCH_LOSS.append(np.mean([epoch_losses[i]['value_loss'] for i in range(len(epoch_losses))]))
       
        # plt.figure()
        # plt.plot(EPOCH_BALANCED_ACCURACY, color='green', label='Validation balanced accuracy')
        # # plt.plot(EPOCH_TRAIN_BALANCED_ACCURACY, color='red', label='Training balanced accuracy')
        # plt.title(f'Balanced accuracy dynamics')
        # plt.xlabel('Epoch')
        # plt.ylabel('Balaned Acuracy')
        # plt.legend()
        # plt.show()
       
        self.epoch_balanced_accuracy = EPOCH_BALANCED_ACCURACY
        self.epoch_mtte = EPOCH_MTTE
        self.epoch_memr = EPOCH_MEMR
        if not self.classify:
            self.epoch_loss = EPOCH_LOSS #[epoch_losses[i]['value_loss'] for i in range(len(epoch_losses))]
        else:
            self.epoch_loss = EPOCH_LOSS
                       
       
        # fig, ax1 = plt.subplots()
        # ax1.set_xlabel('Epoch')
        # ax1.set_ylabel('Validation m-TTE', color='blue')
        # ax1.plot(EPOCH_MTTE[5:], color='blue', label='Validation m-TTE')
        # ax1.grid(False)
        # ax2 = ax1.twinx()  # instantiate a second Axes that shares the same x-axis
        # ax2.set_ylabel('Validation m-EMR', color='orange')  # we already handled the x-label with ax1
        # ax2.plot(EPOCH_MEMR[5:], color='orange', label='Validation m-EMR')
        # ax2.grid(False)
        # lines, labels = ax1.get_legend_handles_labels()
        # lines2, labels2 = ax2.get_legend_handles_labels()
        # ax2.legend(lines + lines2, labels + labels2, loc=0)
        # fig.tight_layout()  # otherwise the right y-label is slightly clipped
        # plt.show()
       
        self.best_crossval_score = best_crossval_score
       
   
       
    def smote(self, data_memory, confidence_score=False, ):
        dat = data_memory.copy()
       
        if self.approx_g:
            try:
                tmp_dat = [dat[i].cpu().detach().numpy() for i in list(dat.keys())]
                dat = dict(zip(list(dat.keys()),tmp_dat))
            except:
                print(f'Elements of dict are already of type np.ndarray')
            y_idx = dat['state_mem'].shape[1]*2 + dat['y'].shape[1]*3
            # print(f"dat['state_mem'].shape: {dat['state_mem'].shape}")
            x_size = dat['state_mem'].shape[1]
            y_size = dat['y'].shape[1]
        else:
            y_idx = dat['state_mem'].shape[1]*2
            x_size = dat['state_mem'].shape[1]
        n_stop_act = max(sum(dat['action_mem'].astype(int)==0)-1,0)
        # print(f'n_stop_act: {n_stop_act}')
        if n_stop_act<=0:
            return dat
        else:
            if self.approx_g:
                df = pd.DataFrame(np.hstack([dat['state_mem'],
                    dat['next_state_mem'],
                    dat['y'].reshape(-1,1), dat['y_next'].reshape(-1,1), dat['y_prev'].reshape(-1,1),
                    dat['action_mem'].astype(int).reshape(-1,1),
                   
                    ]))
                cat_cols = []
                for name in df.columns:
                    unique_count = df[name].unique().shape[0]
                    if unique_count <= 5:
                        cat_cols.append(name)
                self.sm = BOOTSTRAP(
                                 sampling_strategy='minority',
                                    # shrinkage=0.001,
                                # k_neighbors=min(6, n_stop_act)
                                )
                # self.sm = SMOTENC(sampling_strategy='minority',
                #                    k_neighbors=min(6, n_stop_act),
                #                    categorical_features=cat_cols)
            else:
                df = pd.DataFrame(np.hstack([dat['state_mem'],
                        dat['next_state_mem'],
                        dat['action_mem'].astype(int).reshape(-1,1),
                        ]))
                self.sm = SMOTE(sampling_strategy='minority',
                                k_neighbors=min(self.smote_k, n_stop_act)
                                )
                # self.sm = BOOTSTRAP(
                #                   sampling_strategy='minority',
                #                     # shrinkage=0.001,
                #                 # k_neighbors=min(6, n_stop_act)
                #                 )
               
                cat_cols = []
                for name in df.columns:
                    unique_count = df[name].unique().shape[0]
                    if unique_count <= 4:
                        cat_cols.append(name)
               
            df_res, y_resampled = self.sm.fit_resample(df, df.iloc[:,y_idx])
            df_res.loc[y_resampled==0,x_size:x_size*2-1] = df_res.loc[y_resampled==0,:x_size-1].values
            if self.approx_g:
                df_res.loc[y_resampled==0,df_res.columns[-3]] = df_res.loc[y_resampled==0,df_res.columns[-4]].values
               
            dat['state_mem'] = df_res.iloc[:,:x_size].values
            dat['next_state_mem'] = df_res.iloc[:,x_size:x_size*2].values
            if self.approx_g:
                dat['y'] = df_res.iloc[:,-4].values.reshape(-1,1)
                dat['y_next'] = df_res.iloc[:,-3].values.reshape(-1,1)
                dat['y_prev'] = df_res.iloc[:,-2].values.reshape(-1,1)
           
            dat['action_mem'] = y_resampled.values
            dat['confidence_score'] = np.ones_like(df_res.iloc[:,0])
           
            dat['path_ids'] = np.ones_like(df_res.iloc[:,0])
            dat['time_ids'] = np.ones_like(df_res.iloc[:,0])
            dat['done_mem'] = np.zeros_like(df_res.iloc[:,0])
            dat['done_mem'][dat['action_mem']==0] = 1
            dat['init_states_mem'] = df_res.iloc[:,:x_size].values
            dat['reward_mem'] = np.zeros_like(dat['done_mem'])
 
            if confidence_score:
                dat['confidence_score'][~df.index] = self.cs
            st = dat['state_mem']
            act = dat['action_mem']

            return dat
 
   
    def distance_function(self, x):
        return x - 1/2 * x**2
 
    def iq_loss(self, agent, current_Q, current_v, next_v,
                batch, cs, loss_method='value_expert', grad_pen=False,
                lambda_gp=0.1, regularize=True,avg_entropy=False,
                out_degree_weight=None):
        div = self.divergence
        obs, next_obs, action, done = batch[:4]
        done = (action-1)**2
        if self.approx_g:
            y,y_next,y_prev = batch[-3:]
            obs = torch.hstack([obs, y]).clone()
            next_obs = torch.hstack([next_obs, y_next]).clone()
 
        loss_dict = {}
        # keep track of value of initial states
        v0 = self.getV(obs, evaluate=False,
                       out_degree_weight=out_degree_weight.clone().cpu().detach()).mean()
        loss_dict['v0'] = v0.item()
 
        irl_q, q_var = self.infer_q(state=obs, action=action, mc_estimate=avg_entropy,out_degree_weight=out_degree_weight.clone().cpu().detach())
        if self.conservative:
            current_Q = current_Q * torch.hstack([
                (1+out_degree_weight.clone()), 
                (1-out_degree_weight)])    
        # A = 
        Q_entropy = torch.mean(q_var)
        inv_var = 1.6/q_var
        loss_dict['avg_entropy'] = Q_entropy.item()
        #  calculate 1st term for IQ loss
        #  -E_(ρ_expert)[Q(s, a) - γV(s')]
        rew = ((1 - done) * self.gamma * next_v.T.cpu()).T# + 1*done).T

        current_Q_s_a =torch.gather(current_Q.cpu(), dim=-1, index=action.clone().detach().to(torch.int64).cpu().reshape(-1, 1))
        reward = (current_Q_s_a.cpu().clone() - rew.cpu().clone())

        with torch.no_grad():
            # Use different divergence functions (For χ2 divergence we instead add a third bellmann error-like term)
            if div == "hellinger":
                phi_grad = 1/(1+reward)**2
            elif div == "kl":
                # originyal dual form for kl divergence (sub optimal)
                phi_grad = torch.exp(-reward-1)
            elif div == "kl2":
                # biased dual form for kl divergence
                phi_grad = F.softmax(-reward, dim=0) * reward.shape[0]
            elif div == "kl_fix":
                # our proposed unbiased form for fixing kl divergence
                phi_grad = torch.exp(-reward)
            elif div == "js":
                # jensen–shannon
                phi_grad = torch.exp(-reward)/(2 - torch.exp(-reward))
            else:
                phi_grad = 1
       
        if self.IS_CS:
            # td_err = (reward + next_v)- current_Q_s_a
            td_err = -(phi_grad * reward)
            p = torch.abs(td_err) + 0.000001
            P = (p**0.99)/(torch.sum(p**0.99))
            # w = (self.buffer_N*P)**(-self.beta_is)
            w = (P.shape[0]*P)**(-self.beta_is)
            cs = w*td_err
           
        phi_grad=phi_grad*cs
        # if self.conservative:
        #     phi_grad = out_degree_weight*phi_grad     
        
        loss = -(phi_grad * reward).mean()
        # loss = -(self.distance_function(reward)).mean()
        loss_dict['softq_loss'] = loss.item()
 
        # calculate 2nd term for IQ loss, we show different sampling strategies
        if loss_method == "value_expert":
            # sample using only expert states (works offline)
            # E_(ρ)[Q(s,a) - γV(s')]

            value_loss = (cs*(current_v.cpu() - rew.cpu())).mean()
           
            # value_loss = (current_v - y).mean()
            loss += value_loss
            loss_dict['value_loss'] = value_loss.item()
 
        elif loss_method == "value":
            # sample using expert and policy states (works online)
            # E_(ρ)[V(s) - γV(s')]
            value_loss = (current_v.cpu() - rew.cpu()).mean()
            loss += value_loss
            loss_dict['value_loss'] = value_loss.item()
 
        elif loss_method == "v0":
            # alternate sampling using only initial states (works offline but usually suboptimal than `value_expert` startegy)
            # (1-γ)E_(ρ0)[V(s0)]
            v0_loss = (1 - self.gamma) * v0.cpu()
            loss += v0_loss
            loss_dict['v0_loss'] = v0_loss.item()
 
        else:
            raise ValueError(f'This sampling method is not implemented: {loss_method}')
 
        if div == "chi":
            # Use χ2 divergence (calculate the regularization term for IQ loss using expert states) (works offline)
            # y = (1 - done) * gamma * next_v
            y = ((1 - done) * self.gamma * next_v.T.cpu()).T
            reward = cs*(current_Q_s_a.cpu() - rew.cpu())
            chi2_loss = 1/(4 * self.epsilon) * (reward**2).mean()
            loss += chi2_loss
            loss_dict['chi2_loss'] = chi2_loss.item()
 
        if regularize:
            # Use χ2 divergence (calculate the regularization term for IQ loss using expert and policy states) (works online)
            rew = ((1 - done) * self.gamma * next_v.T.cpu()).T
            reward = cs*(current_Q_s_a.cpu() - rew.cpu()) #- (1-done.reshape(-1,1))*y_prev
               
            chi2_loss = 1/(4 * self.epsilon) * (reward**2).mean()
            loss += chi2_loss
            loss_dict['regularize_loss'] = chi2_loss.item()
        
        loss_dict['total_loss'] = loss.clone() #.item()
        return loss, loss_dict
 
    def plot_st_reg_bm(self, test_memory, conservative, discretiser, out_thresh, name):
        if conservative:
            unique_ints, k_outs = discretiser.ints
        import os
        try:
            os.mkdir('./out')
        except:
            pass
        state_mem = test_memory['state_mem']
        def sort_points(xy: numpy.ndarray) -> numpy.ndarray:
            # normalize data  [-1, 1]
            xy_sort = numpy.empty_like(xy)
            xy_sort[:, 0] = 2 * (xy[:, 0] - numpy.min(xy[:, 0]))/(numpy.max(xy[:, 0] - numpy.min(xy[:, 0]))) - 1
            xy_sort[:, 1] = 2* (xy[:, 1] - numpy.min(xy[:, 1])) / (numpy.max(xy[:, 1] - numpy.min(xy[:, 1]))) - 1
   
            # get sort result
            sort_array = numpy.arctan2(xy_sort[:, 0], xy_sort[:, 1])
            sort_result = numpy.argsort(sort_array)
   
            # apply sort result
            return xy[sort_result]
   
       
        fig, (ax2) = plt.subplots(nrows=1, ncols=1, figsize=(30,30))
        # fig.update_layout()
        # fig.layout.update(plot_bgcolor = "white")
        plt.rcParams['axes.facecolor'] = 'white'
        # ax1.scatter(X_resampled[:,1][sel], X_resampled[:,2][sel], s=75)
        ax2.set_xlabel('x2')
        ax2.set_ylabel('x1')
        # ax1.grid(color='grey')
        # ax1.set_title('All stopping coordinates', size=65)
        cmaps = sns.color_palette("magma", 10)
       
        # my_cmap = LinearSegmentedColormap.from_list('my_cmap', cmaps, N=len(numpy.arange(state_mem[:,2].min(),state_mem[:,2].max(), 0.05))+2)
        # print(f'N=:{len(numpy.arange(state_mem[:,2].min(),state_mem[:,2].max(), 0.1))}')
        my_cmap = LinearSegmentedColormap.from_list('my_cmap', cmaps, N=len(numpy.arange(0,50, 5))+5)
        print(f'N=:{len(numpy.arange(0,50, 5))}')
       
        sns.set(font_scale = 4)
       
        # X1_test = numpy.arange(state_mem[:,0].min(),state_mem[:,0].max(), 0.1)
        # X2_test = numpy.arange(state_mem[:,1].min(),state_mem[:,1].max(), 0.1)
        X1_test = numpy.arange(-1.5,1.5, 0.1)
        X2_test = numpy.arange(-1.5,1.5, 0.1)
           
        # for i,j in enumerate(numpy.arange(state_mem[:,2].min(),state_mem[:,2].max(), 0.25)):
        for i,j in enumerate(numpy.arange(0,50, 5)):
            j=j/50
            # print(f'TIME: {round(j,3)}')
            TIME_test = numpy.repeat(j, X1_test.shape[0])
            mesh = numpy.array(numpy.meshgrid(X1_test, X2_test,TIME_test)).T.reshape(-1,3)
            if self.mean.shape[0]==2:
                mesh = numpy.array(numpy.meshgrid(X1_test,TIME_test)).T.reshape(-1,2)
            # print(mesh.shape)
            mesh = torch.Tensor(mesh)
            if conservative:
                state_discretised = discretiser.transform(mesh.cpu().detach().numpy().copy())
                out_degree_test = np.zeros_like(state_discretised[:,0]).astype(float)
                for p in range(len(k_outs)):
                    out_degree_test[(state_discretised.astype(int)==unique_ints[p].astype(int)).all(axis=1)] =  k_outs[p].astype(float)
                with torch.no_grad():
                    out_degree_weight = self.degree_model(torch.from_numpy(out_degree_test).to(torch.float32))
            else:
                out_degree_weight=None
            mesh_norm = (mesh.clone() - self.mean)/self.std

            with torch.no_grad():
               if not self.classify:
                    irl_q0, q0_var =self.infer_q(state=mesh_norm, 
                                                 action=numpy.zeros_like(mesh[:,0]), 
                                                 mc_estimate=self.q_entropy,
                                                 out_degree_weight=out_degree_weight)
                    irl_q1, q1_var = self.infer_q(state=mesh_norm, 
                                                  action=numpy.ones_like(mesh[:,0]), 
                                                  mc_estimate=self.q_entropy,
                                                  out_degree_weight=out_degree_weight)
                    irl_curr_v = self.getV(mesh_norm, 
                                           evaluate=True,
                                           out_degree_weight=out_degree_weight)
               else:
                   self.q_net.eval()
                   with torch.no_grad():
                       irl_q0 = self.q_net(mesh_norm)
                       irl_q1 = 1- irl_q0
                       # q1_var = q0_var
                   irl_q0 = irl_q0.cpu()
                   irl_q1 = irl_q1.cpu()
                   q0_var = irl_q0
                   q1_var = irl_q1
                   irl_curr_v = irl_q0
                

                   # if conservative:
                   #     irl_q0 = (out_degree_weight) + irl_q0.clone().cpu().detach().numpy()
                   #     irl_q1 = (-out_degree_weight) + irl_q1.clone().cpu().detach().numpy()
                   if  self.approx_dynamics:
                       irl_next_s = self.infer_next_s(state=mesh_norm)
                       if conservative:
                           state_discretised_next = discretiser.transform(irl_next_s.cpu().detach().numpy().copy())
                           out_degree_test_next = np.zeros_like(state_discretised_next[:,0]).astype(float)
                           for p in range(len(k_outs)):
                               out_degree_test_next[(state_discretised_next.astype(int)==unique_ints[p].astype(int)).all(axis=1)] =  k_outs[p].astype(float)
                           with torch.no_grad():
                               out_degree_weight_next = self.degree_model(torch.from_numpy(out_degree_test_next).to(torch.float32))
                       else:
                           out_degree_weight_next=None
                       
                       irl_V = self.getV(irl_next_s, evaluate=True,out_degree_weight=out_degree_weight_next)
                       irl_G = irl_q0
                       irl_g = irl_q1 - self.q_net.gamma*irl_V
                   
                       irl_next_q0 = self.infer_q(state=irl_next_s, 
                                                  action=numpy.zeros_like(mesh[:,0]),
                                                  out_degree_weight=out_degree_weight)
                       irl_next_q1 = self.infer_q(state=irl_next_s, 
                                                  action=numpy.ones_like(mesh[:,0]),
                                                  out_degree_weight=out_degree_weight)
                   
                   g_cases = [irl_q1 > irl_q0]
                   G_cases = [irl_q1 <= irl_q0]
                    
                    
            df0 = pd.DataFrame({"x1":mesh[:,0].reshape(-1),
                            "x2":mesh[:,1].reshape(-1),
                            "Q":irl_q0.reshape(-1)})
            df0.x1 = truncate(df0.x1,4)
            df0.x2 = truncate(df0.x2,4)
            # print(f'type(mesh[:,0].reshape(-1)): {type(mesh[:,0].reshape(-1))}')
            data0 = pd.pivot_table(df0, values='Q', index='x1', columns='x2')
            data_norm0 = data0
            data_nan0 = data_norm0
   
            df1 = pd.DataFrame({"x1":mesh[:,0].reshape(-1),
                            "x2":mesh[:,1].reshape(-1),
                            "Q":irl_q1.reshape(-1)})
            df1.x1 = truncate(df1.x1,4).copy()
            df1.x2 = truncate(df1.x2,4).copy()
            
            data1 = pd.pivot_table(df1, values='Q', index='x1', columns='x2')
            data_norm1 = data1.copy()
            data_diff = data_norm0.copy()-data_norm1.copy()
            
            
            try:
                n = data_diff.copy()
                n[n>=0.0]=1
                n[n<0.0]=None
                # sns.heatmap(n)
                nn = n.copy()
                k = numpy.zeros((3,3),dtype=int); k[1] = 1; k[:,1] = 1
                nn[~numpy.isnan(nn)] = 1
                nn[nn != 1] = 0
                nn = nn.astype(int)
                boundary = binary_dilation(nn==0, k) & nn
                boundary = boundary.astype(int)
                boundary[boundary==0] = numpy.nan
                # sns.scatterplot(boundary, ax=ax2)
                newb = boundary.unstack().reset_index()
                newb.columns = ['x2','x1','vals']
                st_points = newb[~numpy.isnan(newb['vals'])]
                coords = st_points[['x1','x2']].values
                # print(i)
                scoords = sort_points(coords)
                tck,u = interpolate.splprep(scoords.transpose(), s=0)
                unew = numpy.arange(0, 1.01, 0.01)
                out = interpolate.splev(unew, tck)
                ax2.plot(out[0], out[1], color=cmaps[i], linewidth=5)
                ax2.grid(color='grey')
            except:
                pass    
        
        plt.xlim(-1.5,1.5)
        plt.ylim(-1.5,1.5)
        plt.title(f'Out-thresh={self.out_thresh}', fontsize=60)
        plt.show()
        
        
        j=25/50
        # print(f'TIME: {round(j,3)}')
        X1_test = numpy.arange(-1.5,1.5, 0.1)
        X2_test = numpy.arange(-1.5,1.5, 0.1)
        TIME_test = numpy.repeat(j, X1_test.shape[0])
        mesh = numpy.array(numpy.meshgrid(X1_test, X2_test,TIME_test)).T.reshape(-1,3)
        if self.mean.shape[0]==2:
            mesh = numpy.array(numpy.meshgrid(X1_test,TIME_test)).T.reshape(-1,2)
        # print(mesh.shape)
        mesh = torch.Tensor(mesh)
        if conservative:
            state_discretised = discretiser.transform(mesh.cpu().detach().numpy().copy())
            out_degree_test = np.zeros_like(state_discretised[:,0]).astype(float)
            for p in range(len(k_outs)):
                out_degree_test[(state_discretised.astype(int)==unique_ints[p].astype(int)).all(axis=1)] =  k_outs[p].astype(float)
            with torch.no_grad():
                out_degree_weight = self.degree_model(torch.from_numpy(out_degree_test).to(torch.float32)).cpu().detach().numpy()
        else:
            out_degree_weight=None
        mesh_norm = (mesh.clone() - self.mean)/self.std
      
        with torch.no_grad():
            
            if not self.classify:
                irl_q0, q0_var =self.infer_q(state=mesh_norm, 
                                             action=numpy.zeros_like(mesh[:,0]), 
                                             mc_estimate=self.q_entropy,
                                             out_degree_weight=torch.from_numpy(out_degree_weight))
                irl_q1, q1_var = self.infer_q(state=mesh_norm, 
                                              action=numpy.ones_like(mesh[:,0]), 
                                              mc_estimate=self.q_entropy,
                                              out_degree_weight=torch.from_numpy(out_degree_weight))
                irl_curr_v = self.getV(mesh_norm, evaluate=True,out_degree_weight=torch.from_numpy(out_degree_weight))
            else:
               self.q_net.eval()
               with torch.no_grad():
                   irl_q0 = self.q_net(mesh_norm)
                   irl_q1 = 1- irl_q0
                   # q1_var = q0_var
               irl_q0 = irl_q0.cpu()
               irl_q1 = irl_q1.cpu()
               q0_var = irl_q0
               q1_var = irl_q1
               irl_curr_v = irl_q0
            
            # if conservative:
            #     irl_q0 = (out_degree_weight)+irl_q0.clone().cpu().detach().numpy()
            #     irl_q1 = (out_degree_weight)+irl_q1.clone().cpu().detach().numpy()
            if  self.approx_dynamics:
                irl_next_s = self.infer_next_s(state=mesh_norm)
                
                if conservative:
                    state_discretised_next = discretiser.transform(irl_next_s.cpu().detach().numpy().copy())
                    out_degree_test_next = np.zeros_like(state_discretised_next[:,0]).astype(float)
                    for p in range(len(k_outs)):
                        out_degree_test_next[(state_discretised_next.astype(int)==unique_ints[p].astype(int)).all(axis=1)] =  k_outs[p].astype(float)
                    with torch.no_grad():
                        out_degree_weight_next = self.degree_model(torch.from_numpy(out_degree_test_next).to(torch.float32))
                else:
                    out_degree_weight_next=None
                    
                irl_V = self.getV(irl_next_s, evaluate=True,out_degree_weight=out_degree_weight_next)
                irl_G = irl_q0
                irl_g = irl_q1 - (self.q_net.gamma*irl_V).cpu().detach().numpy()
               
                irl_next_q0 = self.infer_q(state=irl_next_s, 
                                           action=numpy.zeros_like(mesh[:,0]),
                                           out_degree_weight=torch.from_numpy(out_degree_weight))
                irl_next_q1 = self.infer_q(state=irl_next_s, 
                                           action=numpy.ones_like(mesh[:,0]),
                                           out_degree_weight=torch.from_numpy(out_degree_weight))
               
               
                g_cases = [irl_q1 > irl_q0]
                G_cases = [irl_q1 <= irl_q0]
               
        df0 = pd.DataFrame({"x1":mesh[:,0].reshape(-1),
                        "x2":mesh[:,1].reshape(-1),
                        "Q":irl_q0.reshape(-1)})
        df0.x1 = truncate(df0.x1,4)
        df0.x2 = truncate(df0.x2,4)
        # print(f'type(mesh[:,0].reshape(-1)): {type(mesh[:,0].reshape(-1))}')
        data0 = pd.pivot_table(df0, values='Q', index='x1', columns='x2')
        data_norm0 = data0
        data_nan0 = data_norm0
 
        df1 = pd.DataFrame({"x1":mesh[:,0].reshape(-1),
                        "x2":mesh[:,1].reshape(-1),
                        "Q":irl_q1.reshape(-1)})
        df1.x1 = truncate(df1.x1,4).copy()
        df1.x2 = truncate(df1.x2,4).copy()
        
        data1 = pd.pivot_table(df1, values='Q', index='x1', columns='x2')
        data_norm1 = data1.copy()
        data_diff = data_norm0.copy()-data_norm1.copy()
        
        
        # norm = plt.Normalize(-0.5,0.5)
        # def norm(seq):
        #     extr = max(abs(i) for i in seq)
        #     return [i/extr for i in seq]
            
        from matplotlib import colors
        norm = colors.CenteredNorm(vcenter=0)
        fig, (ax3) = plt.subplots(nrows=1, ncols=1, figsize=(30,30))
        plt.rcParams['axes.facecolor'] = 'white'
        ax3.set_xlabel('x2')
        ax3.set_ylabel('x1')
        cmaps = sns.color_palette("magma", 10)
        sns.heatmap(data0, norm=norm, ax=ax3)
        ax3.set_aspect("equal", 'box')
        ax3.invert_yaxis()
        # plt.xlim(-2,2)
        # plt.ylim(-2,2)
        plt.title('Q0', fontsize=60)
        plt.show()
        
        fig, (ax3) = plt.subplots(nrows=1, ncols=1, figsize=(30,30))
        plt.rcParams['axes.facecolor'] = 'white'
        ax3.set_xlabel('x2')
        ax3.set_ylabel('x1')
        cmaps = sns.color_palette("magma", 10)
        sns.heatmap(data1, norm=norm, ax=ax3)
        ax3.set_aspect("equal", 'box')
        ax3.invert_yaxis()
        # plt.xlim(-2,2)
        # plt.ylim(-2,2)
        plt.title('Q1', fontsize=60)
        plt.show()
        
        fig, (ax3) = plt.subplots(nrows=1, ncols=1, figsize=(30,30))
        plt.rcParams['axes.facecolor'] = 'white'
        ax3.set_xlabel('x2')
        ax3.set_ylabel('x1')
        cmaps = sns.color_palette("magma", 10)
        sns.heatmap(data0-data1, norm=norm, ax=ax3)
        ax3.set_aspect("equal", 'box')
        ax3.invert_yaxis()
        # ax3.set_xlim(-2,2)
        # ax3.set_ylim(-2,2)
        plt.title('Q0-Q1', fontsize=60)
        plt.show()
        

   
def plot_st_reg_car(iq_agent, test_memory):
   
    state_mem = test_memory['state_mem']
    X1_test = numpy.arange(state_mem[:,0].min(),state_mem[:,0].max(), 50)
    X2_test = numpy.arange(state_mem[:,1].min(),state_mem[:,1].max(), 50)
    mesh = numpy.array(numpy.meshgrid(X1_test, X2_test)).T.reshape(-1,2)
    mesh = torch.Tensor(mesh)
    mesh_norm = (mesh.clone()- iq_agent.mean)/iq_agent.std
       
    with torch.no_grad():
        irl_q0 = iq_agent.infer_q(state=mesh_norm, action=numpy.zeros_like(mesh[:,0]))
        irl_q1 = iq_agent.infer_q(state=mesh_norm, action=numpy.ones_like(mesh[:,0]))
        irl_curr_v = iq_agent.getV(mesh_norm, evaluate=True)
        if  iq_agent.approx_dynamics:
            irl_next_s = iq_agent.infer_next_s(state=mesh_norm)
            irl_V = iq_agent.getV(irl_next_s, evaluate=True)
            irl_G = irl_q0
            irl_g = irl_q1 - iq_agent.q_net.gamma*irl_V
           
            irl_next_q0 = iq_agent.infer_q(state=irl_next_s, action=numpy.zeros_like(mesh[:,0]))
            irl_next_q1 = iq_agent.infer_q(state=irl_next_s, action=numpy.ones_like(mesh[:,0]))
            g_cases = [irl_q1 > irl_q0]
            G_cases = [irl_q1 <= irl_q0]
            next_td = torch.zeros_like(irl_curr_v)
            next_td[G_cases] = irl_G[G_cases]
            next_td[g_cases] = (irl_g + torch.maximum(irl_next_q0, irl_next_q1))[g_cases]
            irl_q1 = irl_g + iq_agent.getV(irl_next_s, evaluate=True)
       
        q_0_norm = irl_q0
        df0 = pd.DataFrame({"x1":mesh[:,0].reshape(-1),
                        "x2":mesh[:,1].reshape(-1),
                      "Q":q_0_norm.reshape(-1)})
       
        data0 = pd.pivot_table(df0, values='Q', index='x1', columns='x2')
        data_norm0 = data0
        data_nan0 = data_norm0
 
        q_1_norm = irl_q1
        df1 = pd.DataFrame({"x1":mesh[:,0].reshape(-1),
                        "x2":mesh[:,1].reshape(-1),
                      "Q":q_1_norm.reshape(-1)})
        data1 = pd.pivot_table(df1, values='Q', index='x1', columns='x2')
        data_norm1 = data1
 
 
        data_diff = data_norm0-data_norm1
       
 
        n = data_diff.copy()
        n[n>=0.0]=1
        n[n<0.0]=None
        seaborn.heatmap(n)
        pyplot.show()
